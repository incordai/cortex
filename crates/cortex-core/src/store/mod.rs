...
//! serializes them with the [cortexpack](cortex_pack) format. It is produced and applied through the
...
//! cross-framework stores) lives in the `cortex-store` crate.
...
impl From<cortex_pack::Error> for RecordError {
    fn from(err: cortex_pack::Error) -> Self {
...
impl ModuleRecord {
    ...
    /// Load a record from a cortexpack file on disk.
    #[cfg(feature = "std")]
    pub fn load<P: AsRef<std::path::Path>>(path: P) -> Result<Self, RecordError> {
        Self::from_reader(Reader::from_file(path)?)
    }

    fn pack_tensors(self) -> Vec<cortex_pack::Tensor> {
        self.tensors
            .into_iter()
            .map(|t| {
                cortex_pack::Tensor::new(
                    t.path,
                    t.data.dtype,
                    t.data.shape,
                    Some(t.id.val()),
