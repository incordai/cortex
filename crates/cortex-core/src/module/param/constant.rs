use alloc::format;
use cortex_tensor::kind::{Autodiff, Basic};
...
macro_rules! empty {
    (module) => {
        fn visit<V: cortex::module::ModuleVisitor>(&self, _visitor: &mut V) {
            // Nothing to do
        }

        fn map<M: cortex::module::ModuleMapper>(self, _mapper: &mut M) -> Self {
            self
        }

        fn to_device(self, _: &cortex::tensor::Device) -> Self {
            self
        }

        fn fork(self, _: &cortex::tensor::Device) -> Self {
            self
        }

        fn collect_devices(&self, devices: cortex::module::Devices) -> cortex::module::Devices {
            devices
        }
    };
    ...
    ($type:ty) => {
        impl cortex::module::Module for $type {
            empty!(module);
        }

        impl cortex::module::AutodiffModule for $type {
