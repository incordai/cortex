...
mod grad_distributed {
    use cortex_tensor::TensorData;
    use cortex_tensor::distributed::{DistributedContext, ReduceOperation};
    use cortex_tensor::{Device, DeviceType, Tolerance};
    use rand::{
        SeedableRng,
        rngs::{StdRng, SysRng},
    };
    ...
    fn sharded_module_should_sync_gradients_sum_activation() {
        compare_sync_gradient(ReduceOperation::Sum, |weights, x| {
            let y = weights.clone().matmul(x);
            let y = y.add(weights);
            cortex_tensor::activation::relu(y)
        });
    }
    ...
    fn compare_sync_gradient(
        ...
        use cortex_tensor::distributed::DistributedConfig;
