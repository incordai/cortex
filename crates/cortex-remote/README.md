...
Cortex Remote executes tensor operations on compute peers reached through
...
address lookup, and fleet discovery outside Cortex:

```rust,ignore
use cortex::{Device, Tensor};
use cortex::backend::remote::{Endpoint, RemoteNode, endpoint::presets};
...
Cortex passes the credential to the compute peer's `PeerAuthorizer`; signature format, expiry,
tenant policy, and fleet membership remain application concerns.

## Compute peer

```rust,ignore
use cortex::{Device, server::{self, Channel}};
use cortex::backend::remote::RemoteNode;
...
For an endpoint shared with other Iroh protocols, register Cortex's composable handler in the
application router:

```rust,ignore
use cortex_remote::CORTEX_REMOTE_ALPN;
...
let router = Router::builder(node.endpoint().clone())
    .accept(CORTEX_REMOTE_ALPN, cortex)
