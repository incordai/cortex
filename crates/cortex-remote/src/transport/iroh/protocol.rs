//! Iroh protocol handler for Cortex Remote compute and tensor-transfer streams.

use std::{fmt, sync::Arc};

use cortex_backend::tensor::Device;
use cortex_ir::BackendIr;
use cortex_router::CustomOpRegistry;
use iroh::{
    Endpoint, EndpointId,
...
/// Iroh protocol handler for Cortex Remote compute and tensor-transfer streams.
///
/// Register this handler in an existing Iroh `Router` to compose Cortex with other application
...
/// A backend-erased Cortex Remote protocol handler.
///
...
/// register Cortex on its own Iroh `Router` without naming a backend. Hand it directly to
/// `RouterBuilder::accept` under [`CORTEX_REMOTE_ALPN`](super::node::CORTEX_REMOTE_ALPN).
