...
use crate::transport::iroh::node::CORTEX_REMOTE_ALPN;
...
/// Serve Cortex Remote over Iroh until the process receives its shutdown signal.
///
...
pub(crate) async fn start_iroh_async<B: BackendIr>(
    ...
    let endpoint = Endpoint::builder(presets::N0)
        ...
        .alpns(vec![CORTEX_REMOTE_ALPN.to_vec()])
        .bind()
        .await
        .expect("Can bind the Cortex Remote server endpoint");
    ...
    if let Err(err) = router.shutdown().await {
        log::warn!("Cortex Remote Iroh router shutdown failed: {err}");
    }
}
