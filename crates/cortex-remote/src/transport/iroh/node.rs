//! Process-level Iroh endpoint used by Cortex Remote clients and compute nodes.
...
/// ALPN used by the version-one Cortex Remote protocol.
pub const CORTEX_REMOTE_ALPN: &[u8] = b"cortex/remote/1";
...
/// A process-level Cortex Remote networking node.
///
...
impl RemoteNode {
    ...
    ///
    /// Applications serving Cortex Remote must include [`CORTEX_REMOTE_ALPN`] in the endpoint's
    /// accepted ALPN list, or route that ALPN to Cortex's protocol handler.
    ///
    ...
    pub(crate) async fn accept_stream(
        ...
        if header.version != STREAM_VERSION {
            return Err(format!(
                "Unsupported Cortex Remote stream version {} (expected {STREAM_VERSION})",
                header.version
            ));
        }
        ...
        loop {
            ...
            let connection = cell
                .get_or_try_init(|| async move {
                    endpoint
                        .connect(peer_for_connect.clone(), CORTEX_REMOTE_ALPN)
                        .await
                        .map_err(|err| {
                            format!(
...
pub(crate) async fn send_frame(send: &mut SendStream, bytes: &[u8]) -> Result<(), String> {
    if bytes.len() > MAX_FRAME_SIZE {
        return Err(format!(
            "Cortex Remote frame is too large: {} bytes (max {MAX_FRAME_SIZE})",
            bytes.len()
        ));
    }
...
pub(crate) async fn recv_frame(recv: &mut RecvStream) -> Result<Option<Vec<u8>>, String> {
    ...
    if length > MAX_FRAME_SIZE {
        return Err(format!(
            "Peer sent an oversized Cortex Remote frame: {length} bytes (max {MAX_FRAME_SIZE})"
        ));
    }
