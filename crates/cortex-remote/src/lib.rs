//! Peer-to-peer remote tensor execution for Cortex.
//!
...
pub use cortex_router::RouterClient;
...
#[cfg(feature = "iroh")]
pub use transport::iroh::node::CORTEX_REMOTE_ALPN;
...
mod __client {
    ...
    /// [`Fusion`](cortex_fusion::Fusion) — exactly like the CubeCL backends — so recurring groups of
    ...
    #[cfg(feature = "fusion")]
    pub type RemoteBackend = cortex_fusion::Fusion<BackendRouter<RemoteChannel>>;
...
mod tests {
    ...
    use cortex_tensor::{Device, DeviceType, Distribution, Tensor};
    ...
    pub fn test_custom_op_over_websocket() {
        ...
        use cortex_backend::{Scalar, TensorData, TensorMetadata, ops::FloatTensorOps};
        use cortex_ir::{CustomOpIr, OperationIr, ScalarIr, TensorIr};
    ...
    pub fn test_custom_op_over_iroh() {
        use crate::{
            CORTEX_REMOTE_ALPN, RemoteBackend, RemoteDevice,
        ...
        let router = {
            ...
            Router::builder(server)
                .accept(CORTEX_REMOTE_ALPN, protocol)
                .spawn()
        };
    ...
    fn test_client_disconnect_handled_cleanly_by_server() {
        ...
        type Client = cortex_communication::websocket::WsClient;
...
mod fusion_tests {
    ...
    use cortex_backend::{Backend, Shape, TensorData};
    use cortex_router::BackendRouter;
    ...
    fn run<B: Backend<Device = RemoteDevice>>(
        ...
        for _ in 0..iters {
            ...
            let data = cortex_std::reader::try_read_sync(B::float_into_data(d))
    ...
    fn fusion_matches_plain_remote() {
        ...
        rt.spawn(
            crate::server::RemoteServerBuilder::<cortex_flex::Flex>::new(vec![Default::default()])
                .port(3100)
                .start_async(),
        );
        rt.spawn(
    ...
    fn fusion_custom_source_op_then_followup() {
        ...
        use cortex_flex::Flex;
        use cortex_ir::{CustomOpIr, OperationOutput, ScalarIr, TensorIr};
        ...
        for i in 0..5 {
            ...
            let data = cortex_std::reader::try_read_sync(<RemoteBackend as FloatTensorOps<
                RemoteBackend,
    ...
    fn fusion_custom_source_multi_output_long_lived() {
        ...
        for i in 0..5 {
            ...
            let data =
                cortex_std::reader::try_read_sync(<B as FloatTensorOps<B>>::float_into_data(t))
                    .expect("remote read should resolve synchronously")
                    .expect("read should succeed");
    ...
    fn fusion_custom_source_output_clone_survives_drain() {
        ...
        for i in 0..5 {
            ...
            let exp_data =
                cortex_std::reader::try_read_sync(<B as FloatTensorOps<B>>::float_into_data(exp))
                    .expect("remote read should resolve synchronously")
                    .expect("read should succeed");
            ...
            let log_data =
                cortex_std::reader::try_read_sync(<B as FloatTensorOps<B>>::float_into_data(log))
                    .expect("remote read should resolve synchronously")
                    .expect("read should succeed");
