...
use cortex_ir::{GraphId, NumericOperationIr, OperationIr, TensorId};
...
fn classify_float(op: &cortex_ir::FloatOperationIr) -> OpClass {
    use cortex_ir::FloatOperationIr as F;
    match op {
...
fn classify_module(op: &cortex_ir::ModuleOperationIr) -> OpClass {
    let name = format!("{op:?}");
...
fn classify_base(op: &cortex_ir::BaseOperationIr) -> OpClass {
    use cortex_ir::BaseOperationIr as B;
    match op {
        B::Reshape(_)
        | B::SwapDims(_)
        | B::Permute(_)
