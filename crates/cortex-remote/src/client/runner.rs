...
use cortex_backend::{DeviceId, DeviceOps, ExecutionError, StreamId, TensorData};
use cortex_ir::TensorIr;
use cortex_router::{MultiBackendBridge, RouterClient, RouterTensor, get_client};
use cortex_std::DeviceSettings;
...
impl RouterClient for RemoteClient {
    ...
    fn register_alias(&self, new_id: cortex_ir::TensorId, src_id: cortex_ir::TensorId) {
        let stream_id = StreamId::current();
        self.handle
    ...
    fn register_and_execute_graph(
        &self,
        graph_id: cortex_ir::GraphId,
        relative_graph: Vec<cortex_ir::OperationIr>,
        bindings: cortex_ir::GraphBindings,
    ) {
        let stream_id = StreamId::current();
    ...
    fn execute_graph(&self, graph_id: cortex_ir::GraphId, bindings: cortex_ir::GraphBindings) {
        let stream_id = StreamId::current();
        self.handle
...
impl RemoteClient {
    ...
    fn resolve_devices(&self, mut op: cortex_ir::OperationIr) -> cortex_ir::OperationIr {
        use cortex_ir::{DistributedOperationIr, OperationIr};
...
impl RemoteDevice {
    ...
    pub fn websocket(address: &str, device_index: usize) -> Self {
        let endpoint = RemoteEndpoint::WebSocket {
            address: cortex_communication::Address::from(address),
    ...
    ///
    /// The application owns the Iroh endpoint; Cortex dials the compute peer from it.
    #[cfg(feature = "iroh")]
    ...
    pub fn connect_with_channel<R: cortex_router::RouterChannel<Device = Self>>(&self) {
    ...
    /// behavior of [`Device::enumerate`](cortex_backend::tensor::Device) for local backends.
    #[cfg(feature = "websocket")]
...
impl Default for RemoteDevice {
    fn default() -> Self {
        #[cfg(feature = "websocket")]
        {
            let address = match std::env::var("CORTEX_REMOTE_ADDRESS") {
                Ok(address) => address,
