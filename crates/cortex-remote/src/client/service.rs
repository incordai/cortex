...
///
/// `RemoteService` lives behind a [`DeviceHandle`](cortex_backend::DeviceHandle); every call
...
impl DeviceService for RemoteService {
    fn init(device_id: DeviceId) -> Self {
        ...
        Self {
            ...
            writer: None,
            batch: {
                let cfg = cortex_std::config::config();
            ...
            match reply.content {
                TaskResponseContent::Init(SessionInfo {
                    ...
                    if version != PROTOCOL_VERSION {
                        panic!(
                            "Server uses Cortex Remote protocol version {version}, expected {PROTOCOL_VERSION}"
                        );
                    }
