...
//! session's [`TensorInterpreter`] (with its own [`HandleContainer`](cortex_ir::HandleContainer)),
...
use cortex_ir::{BackendIr, GraphBindings, GraphId};
use cortex_router::{Graph, TensorInterpreter};
    ...
    T: TensorTransfer<B>,
{
    ...
    fn drive(self, receiver: mpsc::Receiver<Task>) {
        ...
        std::thread::Builder::new()
            .name(format!("cortex-remote-session-{session_id}"))
    ...
    ///
    /// Sync work is wrapped in [`StreamId::executes`](cortex_std::id::StreamId::executes) so the
