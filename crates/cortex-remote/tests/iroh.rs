...
use cortex_remote::{
    CORTEX_REMOTE_ALPN, RemoteDevice,
...
fn spawn_router<B: BackendIr>(
    endpoint: Endpoint,
    authorizer: impl cortex_remote::server::PeerAuthorizer,
    ...
    let protocol = IrohRemoteProtocol::<B>::new(
        ...
        probe,
        cortex_remote::server::CustomOpRegistry::default(),
    );
...
async fn passes_application_credentials_to_the_peer_authorizer() {
    ...
    let router = spawn_router::<Flex>(
        server.clone(),
        |request: cortex_remote::server::AuthorizationRequest<'_>| {
...
async fn fused_compute_surfaces_as_graph_telemetry() {
    use cortex_remote::telemetry::{TelemetryEvent, TelemetryProbe, TrafficAggregator};
