...
use cortex_optim::lr_scheduler::module_lr_scheduler::{ModuleLearningRate, ModuleLrScheduler};
use cortex_optim::{GradientsParams, ModuleOptimizer, MultiGradientsParams, OptimizerRecord};
...
impl<M: LearnerModel> Learner<M> {
    ...
    /// step (see [`ModuleOptimizer::load_record`](cortex_optim::ModuleOptimizer::load_record)).
...
impl<M: LearnerModel> LearningCheckpointer<M> {
    ...
    ///
    /// No device is taken: checkpoints are device-free cortexpack records (file-backed bytes). On
