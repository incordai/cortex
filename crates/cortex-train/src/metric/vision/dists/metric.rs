...
use cortex::module::{Content, DisplaySettings, Module, ModuleDisplay, Param};
...
///
/// ```ignore
/// use cortex_train::metric::vision::{DistsConfig, Reduction};
///
...
mod tests {
    ...
    fn test_dists_identical_images_zero_distance() {
        ...
        let image = Tensor::<4>::random(
            [1, 3, 64, 64],
            cortex_core::tensor::Distribution::Uniform(0.0, 1.0),
            &device,
        );
    ...
    fn test_dists_pretrained() {
        ...
        let image1 = Tensor::<4>::random(
            [1, 3, 64, 64],
            cortex_core::tensor::Distribution::Uniform(0.0, 0.3),
            &device,
        );
        let image2 = Tensor::<4>::random(
            [1, 3, 64, 64],
            cortex_core::tensor::Distribution::Uniform(0.7, 1.0),
            &device,
        );
        let distance = dists.forward(image1, image2, Reduction::Mean);
        let distance_value = distance.into_data().to_vec::<f32>().unwrap()[0];
