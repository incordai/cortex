...
use cortex::tensor::ops::{InterpolateMode, InterpolateOptions};
use cortex_nn::conv::{Conv2d, Conv2dConfig};
use cortex_nn::{BatchNorm, BatchNormConfig, PaddingConfig2d};
...
impl InceptionA {
    ...
    pub fn forward(&self, x: Tensor<4>) -> Tensor<4> {
        ...
        let branch_pool =
            cortex_core::tensor::module::avg_pool2d(x, [3, 3], [1, 1], [1, 1], false, false);
...
impl InceptionE {
    ...
    pub fn forward(&self, x: Tensor<4>) -> Tensor<4> {
        ...
        let branch_pool = if self.use_max_pool {
            cortex_core::tensor::module::max_pool2d(x, [3, 3], [1, 1], [1, 1], [1, 1], false)
        } else {
            cortex_core::tensor::module::avg_pool2d(x, [3, 3], [1, 1], [1, 1], false, false)
        };
        let branch_pool = self.branch_pool.forward(branch_pool);
...
impl InceptionV3FeatureExtractor {
    ...
    pub fn forward(&self, x: Tensor<4>) -> Tensor<2> {
        ...
        let x = if h != 299 || w != 299 {
            cortex_core::tensor::module::interpolate(
                x,
                [299, 299],
        ...
        let x = cortex_core::tensor::module::max_pool2d(x, [3, 3], [2, 2], [0, 0], [1, 1], false);
        ...
        let x = cortex_core::tensor::module::adaptive_avg_pool2d(x, [1, 1]);
