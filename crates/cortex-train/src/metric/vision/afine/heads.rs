...
use cortex::tensor::activation::{relu, softplus};
use cortex_nn::{Gelu, Linear, LinearConfig};
...
///
/// Activation is the erf-based [`cortex_nn::Gelu`], not the QuickGELU used
...
impl AfineDHeadConfig {
    ...
    pub fn init(&self, device: &Device) -> AfineDHead {
        ...
        let alpha = Tensor::random(
            [1, 1, D_CHNS_SUM],
            cortex::tensor::Distribution::Normal(0.1, 0.01),
            device,
        );
