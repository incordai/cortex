...
//! is a TorchScript archive that `cortex-store` cannot read, and PyIQA's
...
/// Cached filename inside `~/.cache/cortex-dataset/afine/`.
...
pub(crate) fn load_pretrained_weights(mut afine: Afine) -> Afine {
    ...
    // 0-D scalar tensors (`shape=()`). Cortex's `Param<Tensor<1>>` of
...
/// dict and rename the few fields where the cortex module diverges from
...
/// automatically by `PyTorchToCortexAdapter`.
///
...
/// no corresponding cortex field and is silently dropped via
/// `allow_partial(true)`.
fn load_clip_shard(
    ...
    let mut store = PytorchStore::from_file(cache_path)
        ...
        // No `with_regex` filter: PathFilter matches against cortex-side
        ...
        // corresponding cortex fields and are dropped by
...
/// Load any shard whose keys map directly to cortex-side field names.
