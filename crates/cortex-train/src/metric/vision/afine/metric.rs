...
impl AfineConfig {
    ...
    /// `~/.cache/cortex-dataset/afine/`, and loads all six shards into the
    /// matching submodules.
...
mod tests {
    ...
    fn test_afine_pretrained_parity() {
        ...
        let dis = Tensor::<1, cortex::tensor::Int>::arange(0..total, &device)
            .float()
            .div_scalar((total - 1) as f64)
            .reshape([1, 3, 224, 224]);
        let reference = Tensor::<1, cortex::tensor::Int>::arange(0..total, &device)
            .float()
        ...
        // practice the cortex output matches the captured PyIQA scalar to
