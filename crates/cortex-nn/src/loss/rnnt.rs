...
use cortex::tensor::{Bool, Device, Int, Tensor, s};
...
impl RNNTLoss {
    ...
    pub fn forward(
        ...
        let log_probs = if self.logits {
            ...
            cortex::tensor::activation::log_softmax(logits, vocab_dim)
        } else {
            logits
        };
...
mod tests {
    use super::*;
    use cortex::tensor::{TensorData, Tolerance};
    ...
    fn logits_equivalence() {
        ...
        let logits = Tensor::<4>::from_data(
            cortex_core::tensor::TensorData::new(data, [bs, time_steps, up1, vocab]),
            &dev,
        );
        ...
        let log_probs = cortex::tensor::activation::log_softmax(logits, vocab_dim);
