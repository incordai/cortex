...
fn should_load_bool_from_data_forcing_native_store() {
    use cortex_tensor::{BoolStore, DType};

    // Model loaders (e.g. `cortex-store`) reconstruct tensors with `Tensor::from_data`,
    ...
    // device's default bool storage. Regression test for tracel-ai/cortex#5094.
