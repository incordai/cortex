...
//! `cargo test -p cortex-backend-tests --features ndarray --test tensor report_quantization_accuracy -- --ignored --nocapture`
...
// fourth copy of this table (see also `cortex-std`'s `scale_size` and `cortex-store`'s
// `quant_param_size`).
...
fn scheme_for(value: QuantValue, level: QuantLevel, param: QuantParam) -> QuantScheme {
    let device = cortex_tensor::Device::default();
    device
        .settings()
        .quantization
        .scheme
