...
use cortex_tensor::activation::log_softmax;
use cortex_tensor::module::ctc_loss;
...
fn test_ctc_loss_unreachable_target_is_inf() {
    ...
    // so no valid alignment exists and the loss must be +inf. cortex-nn's
...
fn test_ctc_loss_empty_target() {
    ...
    let expected = cortex_tensor::TensorData::from([3.0f32 * 2.0f32.ln()]);
    loss.into_data()
...
fn test_ctc_loss_long_sequence() {
    ...
    let logits = TestTensor::<3>::from(cortex_tensor::TensorData::new(data, [t, n, c]));
    ...
    let targets = TestTensorInt::<2>::from(cortex_tensor::TensorData::new(tgt_flat, [n, 60]));
...
fn test_ctc_loss_long_mixed_input_lengths() {
    ...
    let mut tgt_flat = tgt_a;
    tgt_flat.extend(tgt_b);
    let targets = TestTensorInt::<2>::from(cortex_tensor::TensorData::new(tgt_flat, [n, max_tgt]));
...
fn test_ctc_loss_narrowed_input() {
    ...
    let full_logits = TestTensor::<3>::from(cortex_tensor::TensorData::new(data, [t_full, n, c]));
    ...
    let reference_data: Vec<FloatElem> = full_log_probs
        ...
        .collect();
    let reference =
        TestTensor::<3>::from(cortex_tensor::TensorData::new(reference_data, [t_eff, n, c]));
...
fn test_ctc_loss_swap_dims_then_narrow() {
    ...
    let logits_btc = TestTensor::<3>::from(cortex_tensor::TensorData::new(data, [b, t_full, c]));
    ...
    let reference_data: Vec<FloatElem> = log_probs_btc
        ...
        .collect();
    let reference =
        TestTensor::<3>::from(cortex_tensor::TensorData::new(reference_data, [t_eff, b, c]));
...
fn test_ctc_loss_matches_pytorch() {
    ...
    let logits =
        TestTensor::<3>::from(cortex_tensor::TensorData::new(data, [t_size, n_size, c_size]));
    let log_probs = log_softmax(logits, 2);
    ...
    let expected = cortex_tensor::TensorData::from([
