...
fn test_whole_max_min_nan_f64_cpu_backends() {
    let tensor = TestTensor::<1>::from([1.0, f32::NAN, 2.0]).cast(cortex_tensor::DType::F64);
...
fn test_whole_max_min_nan_flex_half_dtypes() {
    for dtype in [cortex_tensor::DType::F16, cortex_tensor::DType::BF16] {
...
// All cortex backends should propagate NaN from min/max. See issue #4814.
#[test]
