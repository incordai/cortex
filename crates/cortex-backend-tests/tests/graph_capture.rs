...
//! [`capture`](cortex_tensor::capture) API.
//!
...
use cortex_tensor::{Device, Tolerance};
...
fn capture_replay_matches_eager() {
    ...
    let mut graph = cortex_tensor::capture(&device, || input.clone().mul_scalar(2.0).add_scalar(1.0));
...
fn capture_output_is_stable() {
    ...
    let mut graph = cortex_tensor::capture(&device, || input.clone().add_scalar(10.0));
