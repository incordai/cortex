...
use cortex_tensor::{Element, Tensor, TensorData};
use divan::Bencher;
...
/// `ActivationOps::softmax` default in cortex-backend.
        ...
        mod $mod_name {
            ...
            mod last_axis_f32 {
                ...
                fn trait_hook_bert_seq512_h12(bencher: Bencher) {
                    ...
                    bencher.bench_synced(|| cortex_tensor::activation::softmax(x.clone(), 2));
                }
            ...
            mod non_last_axis_f32 {
                ...
                fn trait_hook_dim1_of_3(bencher: Bencher) {
                    ...
                    bencher.bench_synced(|| cortex_tensor::activation::softmax(x.clone(), 1));
                }
