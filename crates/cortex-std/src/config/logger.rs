...
use super::{
    autodiff::AutodiffLogLevel, base::CortexConfig, fusion::FusionLogLevel, remote::RemoteLogLevel,
};

static CORTEX_LOGGER: Mutex<Option<Logger>> = Mutex::new(None);

#[cfg(feature = "std")]
std::thread_local! {
    static LOCAL_CONFIG: std::cell::OnceCell<Arc<CortexConfig>> =
        const { std::cell::OnceCell::new() };
}

/// Returns the current [`CortexConfig`], cached in thread-local storage on native targets.
///
/// On the first call from a given thread this fetches the global config via
/// [`CortexConfig::get`] (which locks a spin mutex) and caches the `Arc` thread-locally.
/// Subsequent calls on the same thread only pay an `Arc` clone. On `no_std` builds this
/// is equivalent to [`CortexConfig::get`].
///
/// Safe because [`CortexConfig::set`] panics after the first read, so the cached snapshot
/// matches the global singleton for the whole program lifetime.
pub fn config() -> Arc<CortexConfig> {
    #[cfg(feature = "std")]
    {
        LOCAL_CONFIG.with(|cell| cell.get_or_init(CortexConfig::get).clone())
    }
    #[cfg(not(feature = "std"))]
    {
        CortexConfig::get()
...
/// Central logging utility for Cortex, managing one sink registry shared across subsystems.
#[derive(Debug)]
pub struct Logger {
    sinks: LoggerSinks,
    ...
    F: FnOnce() -> String,
{
    ...
    let msg = f();
    let mut guard = CORTEX_LOGGER.lock();
