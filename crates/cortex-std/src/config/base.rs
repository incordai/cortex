...
/// Static mutex holding the global Cortex configuration, initialized as `None`.
static CORTEX_GLOBAL_CONFIG: Mutex<Option<Arc<CortexConfig>>> = Mutex::new(None);

/// Represents the global configuration for Cortex.
...
impl RuntimeConfig for CortexConfig {
    fn storage() -> &'static Mutex<Option<Arc<Self>>> {
        &CORTEX_GLOBAL_CONFIG
    }
    ...
    fn override_from_env(mut self) -> Self {
        ...
        if let Ok(val) = std::env::var("CORTEX_FUSION_LOG") {
        ...
        if let Ok(val) = std::env::var("CORTEX_FUSION_MAX_EXPLORATIONS")
            && let Ok(n) = val.parse::<usize>()
        {
            self.fusion.beam_search.max_explorations = Some(n);
        }

        if let Ok(val) = std::env::var("CORTEX_REMOTE_LOG") {
            let level = match val.to_ascii_lowercase().as_str() {
