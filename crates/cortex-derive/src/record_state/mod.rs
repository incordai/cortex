...
enum FieldKind {
    ...
    VecTensor(Type),
    /// A scalar leaf serialized via `cortex_pack::Scalar`'s `From`/`TryFrom` conversions.
    Scalar,
...
fn is_scalar(ident: &str) -> bool {
    // Only the widths `cortex_pack::Scalar` has `From`/`TryFrom` conversions for. `i128`/`u128` are
...
fn try_derive_impl(ast: &DeriveInput) -> Result<proc_macro::TokenStream, syn::Error> {
    ...
    for field in fields {
        ...
        match classify(ty)? {
            ...
            FieldKind::VecTensor(inner) => {
                flatten.push(quote! {
                    out.push_scalar(
                        prefix,
                        concat!(#leaf, ".len"),
                        cortex_pack::Scalar::from(self.#ident.len()),
                    );
                ...
                unflatten.push(quote! {
                    let #ident = {
                        let __len = <usize as ::core::convert::TryFrom<cortex_pack::Scalar>>::try_from(
            ...
            FieldKind::Scalar => {
                flatten.push(quote! {
                    out.push_scalar(prefix, #leaf, cortex_pack::Scalar::from(self.#ident));
                });
                unflatten.push(quote! {
                    let #ident = <#ty as ::core::convert::TryFrom<cortex_pack::Scalar>>::try_from(
            ...
            FieldKind::OptionScalar(inner) => {
                flatten.push(quote! {
                    if let Some(value) = self.#ident {
                        out.push_scalar(prefix, #leaf, cortex_pack::Scalar::from(value));
                    }
                });
                unflatten.push(quote! {
                    let #ident: Option<#inner> = src.take_scalar(prefix, #leaf).and_then(|__s| {
                        <#inner as ::core::convert::TryFrom<cortex_pack::Scalar>>::try_from(__s).ok()
                    });
                });
            }
            FieldKind::Nested => {
