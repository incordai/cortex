...
impl ModuleCodegen for StructModuleCodegen {
    fn gen_num_params(&self) -> TokenStream {
        let body = self.gen_fields_fn(|name, field_type| {
            if field_type.is_parameter_module() || field_type.maybe_generic_module() {
                quote! {
                    num_params += cortex::module::Module::num_params(&self.#name);
                }
            } else {
    ...
    fn gen_visit(&self) -> TokenStream {
        ...
        quote! {
            fn visit<Visitor: cortex::module::ModuleVisitor>(&self, visitor: &mut Visitor) {
                #body
            }
        }
    }
    ...
    fn gen_collect_devices(&self) -> TokenStream {
        let body = self.gen_fields_fn(|name, field_type| {
            if field_type.is_module || field_type.maybe_generic_module() {
                quote! {
                    let devices = cortex::module::Module::collect_devices(&self.#name, devices);
                }
            } else {
                quote! {}
            }
        ...
        quote! {
            fn collect_devices(
                &self,
                devices: cortex::module::Devices
            ) -> cortex::module::Devices {
                #body
                devices
            }
        }
    ...
    fn gen_to_device(&self) -> TokenStream {
        let (names, body) = self.gen_fields_fn_names(|name, field_type| {
            if field_type.is_module || field_type.maybe_generic_module() {
                quote! {
                    let #name = cortex::module::Module::to_device(self.#name, device);
                }
            } else {
        ...
        quote! {
            fn to_device(self, device: &cortex::tensor::Device) -> Self {
                #body
                Self { #(#names),* }
            }
        }
    ...
    fn gen_fork(&self) -> TokenStream {
        let (names, body) = self.gen_fields_fn_names(|name, field_type| {
            if field_type.is_module || field_type.maybe_generic_module() {
                quote! {
                    let #name = cortex::module::Module::fork(self.#name, device);
                }
            } else {
                quote! { let #name = self.#name; }
            }
        ...
        quote! {
            fn fork(self, device: &cortex::tensor::Device) -> Self {
                #body
                Self { #(#names),* }
            }
        }
    ...
    fn gen_map(&self) -> TokenStream {
        ...
        let (names, body) = self.gen_fields_fn_names(|name, field_type| {
            if field_type.is_parameter_module() || field_type.maybe_generic_module() {
                ...
                quote! {
                    ...
                    let #name = cortex::module::Module::map(self.#name, mapper);
        ...
        quote! {
            fn map<Mapper: cortex::module::ModuleMapper>(self, mapper: &mut Mapper) -> Self {
                #body
                Self { #(#names),* }
            }
        }
    ...
    fn gen_valid(&self) -> TokenStream {
        let (names, body) = self.gen_fields_fn_names(|name, field_type| {
            if field_type.is_module || field_type.maybe_generic_module() {
                quote! {
                    let #name = cortex::module::AutodiffModule::valid(&self.#name);
                }
            } else {
    ...
    fn gen_from_inner(&self) -> TokenStream {
        let (names, body) = self.gen_fields_fn_names(|name, field_type| {
            if field_type.is_module || field_type.maybe_generic_module() {
                quote! {
                    let #name = cortex::module::AutodiffModule::from_inner(#name);
                }
            } else {
                quote! { let #name = #name; }
            }
...
fn type_matches_ident(ty: &syn::Type, idents: &[&str]) -> bool {
    if let syn::Type::Path(type_path) = ty {
        // Look at the last segment of the path (e.g., 'Param' in 'cortex::module::Param')
