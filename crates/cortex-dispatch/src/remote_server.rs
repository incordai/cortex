//! Concrete-backend dispatch for `cortex-remote`'s server entry points.
//!
//! Lives in `cortex-dispatch` because matching on [`DispatchDevice`] requires the
...
//! surface (`Channel` enum, opaque `Device` argument) lives in `cortex-tensor`.

use std::sync::Arc;

use cortex_remote::Endpoint;
use cortex_remote::server::{CustomOpRegistry, IrohRemoteProtocol, PeerAuthorizer, RemoteProtocol};
use cortex_remote::telemetry::TelemetryProbe;

use crate::backends::*;
use crate::{Dispatch, DispatchDevice, DispatchDeviceId};

/// Transport used to serve remote clients. Re-exported from `cortex-remote` so the whole stack
/// shares one definition.
pub use cortex_remote::server::Channel;
...
pub fn start(device: DispatchDevice, channel: Channel) {
    with_backend!(device, |B, devices| {
        cortex_remote::server::RemoteServerBuilder::<B>::new(devices)
            .channel(channel)
            .start()
    })
}
...
/// the caller registers on its own Iroh router under [`CORTEX_REMOTE_ALPN`].
