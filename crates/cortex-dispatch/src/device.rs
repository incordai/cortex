use cortex_backend::{DeviceId, DeviceOps, DeviceSettings};
...
// Throughput types come from `cortex-backend` (which re-exports them from cubecl),
// so `cortex-dispatch` needs no direct `cubecl` dependency.
#[cfg(feature = "cubecl")]
use alloc::vec::Vec;
#[cfg(feature = "cubecl")]
use cortex_backend::cubecl::{ThroughputKey, ThroughputValue};
...
impl DispatchDevice {
    ...
    /// [`ThroughputValue`](cortex_backend::cubecl::ThroughputValue) corresponds
    ...
    pub fn performance_stats(&self, keys: &[ThroughputKey]) -> Vec<ThroughputValue> {
        ...
        match self {
            #[cfg(feature = "cpu")]
            DispatchDevice::Cpu(device) => cortex_cpu::device_throughput(device, keys),
            #[cfg(feature = "cuda")]
            DispatchDevice::Cuda(device) => cortex_cuda::device_throughput(device, keys),
            #[cfg(feature = "rocm")]
            DispatchDevice::Rocm(device) => cortex_rocm::device_throughput(device, keys),
            #[cfg(feature = "wgpu")]
            DispatchDevice::Wgpu(device) => cortex_wgpu::device_throughput(device, keys),
            #[cfg(feature = "vulkan")]
            DispatchDevice::Vulkan(device) => cortex_wgpu::device_throughput(device, keys),
            #[cfg(feature = "metal")]
            DispatchDevice::Metal(device) => cortex_wgpu::device_throughput(device, keys),
            #[cfg(feature = "webgpu")]
            DispatchDevice::WebGpu(device) => cortex_wgpu::device_throughput(device, keys),
...
impl Default for DispatchDevice {
    ...
    fn default() -> Self {
        // TODO: which priority?
        // Single override e.g. `CORTEX_DEVICE=vulkan` forces Vulkan or panics if not available.
        // Priority list e.g. `CORTEX_DEVICE_PRIORITY=cuda,vulkan,cpu` sets the order.
        // Both could be tied into `cortex.toml` config
        // For now we just use `CORTEX_DEVICE` on CI to force a single device

        #[cfg(feature = "std")]
        {
            if let Ok(device_str) = std::env::var("CORTEX_DEVICE") {
                match device_str.to_lowercase().as_str() {
                    "cuda" => {
                        #[cfg(feature = "cuda")]
                        return Self::Cuda(CudaDevice::default());
                        panic!(
                            "CORTEX_DEVICE=cuda requested, but the 'cuda' feature is not enabled."
                        );
                    }
                    "metal" => {
                        #[cfg(feature = "metal")]
                        ...
                        panic!(
                            "CORTEX_DEVICE=metal requested, but the 'metal' feature is not enabled."
                        );
                    }
                    "rocm" => {
                        #[cfg(feature = "rocm")]
                        ...
                        panic!(
                            "CORTEX_DEVICE=rocm requested, but the 'rocm' feature is not enabled."
                        );
                    }
                    "vulkan" => {
                        #[cfg(feature = "vulkan")]
                        ...
                        panic!(
                            "CORTEX_DEVICE=vulkan requested, but the 'vulkan' feature is not enabled."
                        );
                    }
                    "webgpu" => {
                        #[cfg(feature = "webgpu")]
                        ...
                        panic!(
                            "CORTEX_DEVICE=webgpu requested, but the 'webgpu' feature is not enabled."
                        );
                    }
                    "wgpu" => {
                        #[cfg(feature = "wgpu")]
                        return Self::Wgpu(cortex_wgpu::WgpuDevice::default());
                        panic!(
                            "CORTEX_DEVICE=wgpu requested, but the 'wgpu' feature is not enabled."
                        );
                    }
                    "cpu" => {
                        #[cfg(feature = "cpu")]
                        return Self::Cpu(CpuDevice);
                        panic!("CORTEX_DEVICE=cpu requested, but the 'cpu' feature is not enabled.");
                    }
                    "tch" => {
                        #[cfg(feature = "tch")]
                        return Self::LibTorch(LibTorchDevice::default());
                        panic!("CORTEX_DEVICE=tch requested, but the 'tch' feature is not enabled.");
                    }
                    "remote" => {
                        #[cfg(feature = "remote")]
                        return Self::Remote(RemoteDevice::default());
                        panic!(
                            "CORTEX_DEVICE=remote requested, but the 'remote' feature is not enabled."
                        );
                    }
                    "flex" => {
                        #[cfg(any(feature = "flex", default_backend))]
                        ...
                        panic!(
                            "CORTEX_DEVICE=flex requested, but the 'flex' feature is not enabled."
                        );
                    }
                    "ndarray" => {
                        #[cfg(feature = "ndarray")]
                        ...
                        panic!(
                            "CORTEX_DEVICE=ndarray requested, but the 'ndarray' feature is not enabled."
                        );
                    }
                    _ => panic!("Unknown CORTEX_DEVICE override: '{}'.", device_str),
                }
            }
        }
...
impl cortex_backend::Device for DispatchDevice {
