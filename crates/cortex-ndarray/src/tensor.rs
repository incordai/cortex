...
/// When data is borrowed from external sources (like cortexpack files),
...
impl NdArrayTensor {
    ...
    pub fn from_data(data: TensorData) -> NdArrayTensor {
        // Only use Borrowed storage for non-native allocations (e.g., cortexpack mmap/file).
...
mod tests {
    ...
    fn zero_copy_creates_borrowed_storage_for_non_native() {
        ...
        // (e.g. cortexpack mmap/file data tagged with AllocationProperty::Other or File).
        // Native heap allocations intentionally use Owned storage for performance.
        use cortex_backend::AllocationProperty;
        use cortex_std::Bytes;

        let data: Vec<f32> = vec![1.0, 2.0, 3.0, 4.0];
        let bytes = Bytes::from_elems(data);
        // Tag as Other to simulate cortexpack / mmap data (non-native backing storage)
            ...
            NdArrayTensor::F32(storage) => {
                assert!(
                    ...
                    "ZERO-COPY REGRESSION: from_data should create borrowed storage \
                     for non-native (e.g. cortexpack) TensorData"
                );
                assert!(
                    !storage.is_unique(),
