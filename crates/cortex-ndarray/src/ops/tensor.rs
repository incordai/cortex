...
use cortex_backend::{ElementConversion, Shape, TensorData, ops::FloatTensorOps};
...
impl FloatTensorOps<Self> for NdArray {
    ...
    fn float_scatter(
        ...
        indices: NdArrayTensor,
        value: FloatTensor<Self>,
        update: cortex_backend::tensor::IndexingUpdateOp,
    ) -> FloatTensor<Self> {
        match update {
            cortex_backend::tensor::IndexingUpdateOp::Add => {
                Self::float_scatter_add(dim, tensor, indices, value)
            }
            cortex_backend::tensor::IndexingUpdateOp::Assign => {
                execute_with_int_dtype!(
                    indices,
                    IntElem,
                    |idx_array: SharedArray<IntElem>| -> NdArrayTensor {
