...
//! from cortexpack files. When data is borrowed from external memory (like mmap'd files
...
///
/// - **Borrowed**: Data from external source (cortexpack, mmap, static).
...
pub enum NdArrayStorage<E: Element> {
    /// Borrowed from external source (e.g., cortexpack zero-copy load).
...
impl<E: Element> NdArrayStorage<E> {
    ...
    ///
    /// These requirements are upheld when loading from `TensorData` (cortexpack, etc.)
...
mod tests {
    ...
    fn test_zero_copy_shared_bytes_pointer_identity() {
        ...
        // This simulates the actual cortexpack/mmap loading path.
        use cortex_std::AllocationProperty;
