...
use cortex_backend::{Backend, BackendTypes, DType, DeviceId, DeviceOps};
use cortex_ir::{BackendIr, HandleKind, TensorHandle};
use cortex_std::sync::Mutex;
use cortex_std::{BoolStore, DeviceSettings, QuantConfig};
...
impl Backend for NdArray {
    ...
    fn dtype_usage(_device: &Self::Device, dtype: DType) -> cortex_backend::DTypeUsageSet {
        match dtype {
            ...
            | DType::U16
            | DType::U8
            | DType::Bool(BoolStore::Native) => cortex_backend::DTypeUsage::general(),
            DType::F16 | DType::BF16 | DType::Bool(_) => cortex_backend::DTypeUsageSet::empty(),
            DType::QFloat(scheme) => {
                match scheme {
                    QuantScheme {
