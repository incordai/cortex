# cortex-flex vs cortex-ndarray: Comprehensive Comparison

This document compares cortex-flex (proposed replacement) against cortex-ndarray (current CPU backend)
to demonstrate full coverage and the architectural differences between the two.

## Executive Summary

cortex-flex is a from-scratch CPU backend built to replace cortex-ndarray. The
...
simulates quantization rather than executing natively. cortex-flex addresses all of these while
passing the full `cortex-backend-tests` suite, all ONNX model checks, and real model inference
(ALBERT, MiniLM).
...
- **Structural improvements** (up to 166,000x): Operations that cortex-ndarray eagerly materializes
  (unfold, expand, slice, dequantize) are represented as zero-copy views or direct lookups in
  cortex-flex, avoiding the work entirely.

cortex-flex uses significantly less memory, supports f16/bf16 natively, runs on no_std/WASM/embedded,
...
| Aspect      | cortex-flex                                             | cortex-ndarray                                                              |
| ----------- | ----------------------------------------------------- | ------------------------------------------------------------------------- |
...
| Dtype    | cortex-flex                                                   | cortex-ndarray          |
| -------- | ----------------------------------------------------------- | --------------------- |
...
cortex-flex's f16 support is native for all operations. For matmul and convolution, the gemm crate has
...
| Feature    | cortex-flex                 | cortex-ndarray                            |
| ---------- | ------------------------- | --------------------------------------- |
...
| Operation                                                         | cortex-flex | cortex-ndarray | Notes                                                 |
| ----------------------------------------------------------------- | --------- | ------------ | ----------------------------------------------------- |
| from_data                                                         | Yes       | Yes          |                                                       |
| into_data                                                         | Yes       | Yes          |                                                       |
| random                                                            | Yes       | Yes          |                                                       |
...
cortex-ndarray's dimension limit comes from its `reshape!` macro which matches on dimensions 1-6:
...
cortex-flex uses `IxDyn`-equivalent dynamic shapes with no upper bound.

---

## 5. Zero-Copy Operations

| Operation      | cortex-flex                         | cortex-ndarray                   |
| -------------- | --------------------------------- | ------------------------------ |
...
| Strategy           | cortex-flex                                             | cortex-ndarray                       |
| ------------------ | ----------------------------------------------------- | ---------------------------------- |
...
Both backends support zero-copy loading from external sources (cortexpack files, mmap'd data):
...
cortex-ndarray offers optional BLAS acceleration (Accelerate on macOS, OpenBLAS, Netlib) through
feature flags. cortex-flex uses only the gemm crate, which is pure Rust but highly optimized with its
...
| Target                          | cortex-flex                          | cortex-ndarray              |
| ------------------------------- | ---------------------------------- | ------------------------- |
| x86_64 (std)                    | Yes                                | Yes                       |
...
cortex-flex has been explicitly tested on embedded targets with Cortex's `cortex-no-std-tests` integration
...
| ------------ | ----------------------------------- | ------------------ |
| cortex-backend | Backend traits, types               | Always             |
| cortex-ir      | BackendIr trait                     | Always             |
| cortex-std     | Bytes, Shape, platform abstractions | Always             |
...
| -------------------- | -------------------------------- | -------------------------- |
| cortex-backend         | Backend traits, types            | Always                     |
| cortex-std             | Platform abstractions            | Always                     |
| cortex-autodiff        | Autodiff support                 | Optional (`std`)           |
| cortex-ir              | IR types                         | Always                     |
...
cortex-flex has significantly fewer dependencies, with no dependency on ndarray itself, no macro
...
| Metric         | cortex-flex     | cortex-ndarray |
| -------------- | ------------- | ------------ |
| Source files   | 38            | 37           |
...
cortex-flex has roughly 2x the code. This is because:
...
| Aspect                | cortex-flex                                                  | cortex-ndarray            |
| --------------------- | ---------------------------------------------------------- | ----------------------- |
| cortex-backend-tests    | **All pass** (6 feature flag combos)                       | All pass                |
| cortex-no-std-tests     | **Pass** (MNIST inference)                                 | Not explicitly verified |
...
cortex-ndarray eagerly materializes data where cortex-flex uses zero-copy views or separated storage.
...
> **Note on quantization**: cortex-ndarray simulates quantization by dequantizing to f32 for most
...
cortex-flex was built to address these gaps without waiting on upstream. It is not intended to compete
with CubeCL CPU, which targets high-performance computation through operator fusion and just-in-time
compilation via LLVM. The goal is to provide a lightweight, portable replacement for cortex-ndarray
that works today on platforms CubeCL CPU cannot target (no_std, WASM, embedded).

## 17. What cortex-flex Adds
...
## 18. What cortex-ndarray Has That cortex-flex Does Not

1. **BLAS acceleration**: Feature flags for Accelerate (macOS), OpenBLAS, and Netlib BLAS. These can
   outperform gemm for very large matmuls on specific hardware. cortex-flex relies solely on the gemm
   crate.

2. **SIMD conv/pool kernels**: cortex-ndarray has dedicated macerator-based SIMD kernels for
   convolution and pooling. cortex-flex delegates to gemm's SIMD.

3. **export_tests feature**: cortex-ndarray serves as a reference implementation for some cortex-cubecl
...
For Cortex users switching from cortex-ndarray to cortex-flex:
...
| Autodiff       | Use `cortex_autodiff::Autodiff<Flex>` (same pattern) |
| f16/bf16       | Works out of the box (new capability)              |
| Quantization   | Same API, faster execution                         |
| Tests          | Same cortex-backend-tests suite passes               |

---

## 20. Conclusion

cortex-flex is a from-scratch replacement for cortex-ndarray, motivated by ndarray's lack of f16/bf16
