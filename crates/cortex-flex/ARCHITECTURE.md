...
cortex-flex is tested for edge-case robustness to ensure safe behavior on embedded devices and in
...
1. **Leverage Cortex** - Use `cortex-backend` types and `cortex-std` utilities wherever possible
...
4. **Simple and direct** - Eager execution, no lazy graphs, no fusion (use `cortex-fusion` if needed)
...
Measured via `cargo bench -p cortex-flex --bench {matmul,attention,conv_ops}` with features
...
cortex-flex does not implement `FusionBackend`. Without JIT compilation, fusion adds tracking overhead
with no performance benefit. Deferred operations would still execute one-by-one with intermediate
allocations. For CPU with fusion, use `cortex-cpu` (which has cubecl's MLIR-based JIT runtime).
...
- Matches Cortex's IntElem type for compatibility
...
| **Kernel fusion**                | Outside cortex-flex scope. Fusion is handled at the Cortex framework level via `cortex-fusion`. This backend focuses on single-operation efficiency.                                                |
...
`Bytes` from cortex-std supports zero-copy scenarios (mmap, external buffers). `FlexTensor` wraps this
