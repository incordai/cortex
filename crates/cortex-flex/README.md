...
A fast, memory-efficient CPU backend for Cortex with multi-threading, SIMD, and optimized matrix
multiplication. Runs on std, no_std, and WebAssembly. Supports f16/bf16, zero-copy data loading, and
is thread-safe by design.

> **[Detailed comparison with cortex-ndarray](./COMPARISON.md)**: Full architecture, feature coverage,
...
- **Built on Cortex**: Leverages Cortex's native infrastructure (`Bytes`, `Shape`, `TensorData`,
  `Element` trait) from cortex-backend and cortex-std
...
Enable opt-in paths by passing them to Cargo, either directly on `cortex-flex` or through the
top-level `cortex` crate:

```toml
# Direct
cortex-flex = { version = "0.21", features = ["apple-amx"] }

# Via cortex
cortex = { version = "0.21", features = ["flex", "apple-amx"] }
```

See [ARCHITECTURE.md#feature-flags](./ARCHITECTURE.md#feature-flags) for per-case benchmark impact.

### Why replace cortex-ndarray?
...
cortex-flex was built as a from-scratch replacement that addresses the gaps while maintaining full
compatibility with Cortex's backend test suite.

### Performance vs cortex-ndarray (Apple M3 Max)

#### Compute Performance

cortex-ndarray now uses macerator SIMD for f32 elementwise ops
...
These reflect better _operation representation_, not faster computation. cortex-ndarray eagerly
materializes data for these operations; cortex-flex avoids the work entirely through zero-copy views
...
- All `cortex-backend-tests` pass across all feature flag combinations:
...
- Cortex's `cortex-no-std-tests` integration suite passes (MNIST model inference in `#![no_std]`)
...
- Passes [Miri](https://github.com/rust-lang/miri) (undefined behavior detector) on all burn-flex
  code (quantization tests skipped due to upstream UB in cortex-std). Validates memory safety of
...
- All ONNX model checks in `cortex-onnx` pass
...
- [COMPARISON.md](./COMPARISON.md) - Comprehensive comparison with cortex-ndarray
...
- [ACKNOWLEDGMENTS.md](./ACKNOWLEDGMENTS.md) - Projects that influenced cortex-flex
