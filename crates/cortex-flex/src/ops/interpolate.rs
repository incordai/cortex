...
where
    T: Float + cortex_backend::Element + bytemuck::Pod + Send + Sync,
{
    ...
    T: Float + cortex_backend::Element + bytemuck::Pod + Send + Sync,
{
    ...
    #[inline]
    fn bicubic_sample<T: Float + cortex_backend::Element + bytemuck::Pod>(
        input: &[T],
        in_base: usize,
        in_width: usize,
        y_taps: &[AxisTap; 4],
    ...
    T: Float + cortex_backend::Element + bytemuck::Pod + Send + Sync,
{
    ...
    #[inline]
    fn lanczos3_sample<T: Float + cortex_backend::Element + bytemuck::Pod>(
        input: &[T],
        in_base: usize,
        in_width: usize,
        y_taps: &AxisTaps<6>,
...
where
    T: Float + cortex_backend::Element + bytemuck::Pod,
{
    let grad = grad.to_contiguous();
    let grad_data = grad.storage::<T>();
    let shape = x.layout().shape();
...
// under crates/cortex-backend-tests/tests/tensor/float/module/{bicubic,bilinear,
...
mod tests {
        ...
        T: Float + cortex_backend::Element + bytemuck::Pod + Send + Sync,
    {
        ...
        let input = FlexTensor::new(
            ...
            <T as cortex_backend::Element>::dtype(),
        );
