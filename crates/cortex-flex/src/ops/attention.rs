...
fn broadcast_attn_mask_bias(
    ...
    if src[2] != target[2] || src[3] != target[3] {
        let expanded = crate::ops::expand::expand(tensor, cortex_std::Shape::new(target));
...
fn cast_to_f32<E: cortex_backend::Element + Pod + Copy>(
    tensor: FlexTensor,
    to_f32: fn(E) -> f32,
) -> FlexTensor {
...
fn cast_from_f32<E: cortex_backend::Element + Pod + Copy>(
    tensor: FlexTensor,
    from_f32: fn(f32) -> E,
) -> FlexTensor {
    let tensor = tensor.to_contiguous();
...
where
    T: FlashGemm + cortex_backend::Element,
{
    ...
    let shape = cortex_std::Shape::from(vec![batch, heads, seq_q, val_dim]);
    FlexTensor::new(
...
// crates/cortex-backend-tests/tests/tensor/float/module/attention.rs, which
...
mod tests {
    ...
    use cortex_backend::DType;
    use cortex_backend::ops::AttentionModuleOptions;
    use cortex_std::{BoolStore, Bytes, Shape, f16};
    ...
    /// repeated `q_heads / kv_heads` times - ONNX Attention-23 / cortex-ndarray
    ...
    fn test_flash_bias_broadcast_across_batch_and_heads() {
        ...
        // crates/cortex-backend-tests/tests/tensor/float/module/attention.rs.
        let batch = 2;
        let heads = 2;
        let seq_q = 3;
        let seq_kv = 5;
