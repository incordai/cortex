...
use cortex_backend::ops::conv::calculate_conv_output_size;
use cortex_std::{Bytes, Shape, f16};
...
fn conv3d_impl<
    T: bytemuck::Pod + Clone + Copy + cortex_backend::Element + cortex_backend::ElementAdd + Send + Sync,
>(
    x: FlexTensor,
    weight: FlexTensor,
    bias: Option<FlexTensor>,
...
fn conv_plane_accumulate_oh_outer<T: num_traits::Float + Copy>(
    ...
    // from the in-tree callers (cortex's `calculate_conv_output_size` is
...
where
    T: num_traits::Float
        ...
        + Clone
        + Copy
        + cortex_backend::Element
        + cortex_backend::ElementAdd
        + Send
        + Sync,
{
...
/// and keep their 1.1-1.4x win over cortex-ndarray.
...
// API level by crates/cortex-backend-tests/tests/tensor/float/module/
