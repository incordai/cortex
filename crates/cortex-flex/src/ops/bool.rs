...
use cortex_std::{Bytes, FloatDType, IntDType, Shape, Slice, bf16, f16};
...
impl BoolTensorOps<Flex> for Flex {
    ...
    fn bool_into_int(tensor: BoolTensor<Flex>, out_dtype: cortex_std::IntDType) -> IntTensor<Flex> {
    ...
    fn bool_equal(lhs: BoolTensor<Flex>, rhs: BoolTensor<Flex>) -> BoolTensor<Flex> {
        ...
        let out_dtype = cortex_std::BoolDType::from(lhs.dtype());
    ...
    fn bool_not(mut tensor: BoolTensor<Flex>) -> BoolTensor<Flex> {
        ...
        let out_dtype = cortex_std::BoolDType::from(tensor.dtype());
        let shape = tensor.layout().shape().clone();
    ...
    fn bool_equal_elem(lhs: BoolTensor<Flex>, rhs: cortex_backend::Scalar) -> BoolTensor<Flex> {
        use crate::strided_index::StridedIter;
    ...
    fn bool_not_equal_elem(lhs: BoolTensor<Flex>, rhs: cortex_backend::Scalar) -> BoolTensor<Flex> {
        let out_dtype = cortex_std::BoolDType::from(lhs.dtype());
...
// to crates/cortex-backend-tests/tests/tensor/bool/ops/{logical,cast}.rs
...
mod tests {
    ...
    use cortex_std::{FloatDType, IntDType};
