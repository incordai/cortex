...
//! replacing the default multi-op compositions from Cortex's trait defaults.

use alloc::vec;
use alloc::vec::Vec;
use cortex_backend::Scalar;
use cortex_backend::ops::{ActivationOps, FloatTensorOps};
...
// `cortex_tensor::activation::softmax` falls back to a 5-op decomposition
...
// ============================================================================
//
// `cortex::nn::LayerNorm::forward` decomposes into ~6 primitive tensor ops
...
pub fn layer_norm(
    ...
    match input.dtype() {
        ...
        dtype => panic!("cortex_flex::layer_norm: unsupported dtype {:?}", dtype),
    }
}

fn layer_norm_via_f32<E: cortex_backend::Element + bytemuck::Pod + Copy>(
    input: FlexTensor,
    gamma: FlexTensor,
...
fn layer_norm_row_f32_simd<S: macerator::Simd>(
    ...
    let n = len as f32;
    let mean = sum / n;
    // Biased variance: E[x^2] - E[x]^2. Matches cortex::nn::LayerNorm which
...
// migrated to cortex-backend-tests so they cover every backend. When adding
// new tests, keep them here only if they probe flex internals; otherwise
// add them to crates/cortex-backend-tests/tests/tensor/float/activation/.
#[cfg(test)]
mod tests {
    use alloc::vec;
    use cortex_backend::{DType, TensorData, TensorMetadata, Tolerance};
    ...
    fn flex_half<T: cortex_backend::Element>(data: Vec<T>, shape: &[usize]) -> FlexTensor {
        FlexTensor::from_data(TensorData::new(data, shape.to_vec()))
    }
    ...
    where
        E: cortex_backend::Element + Float,
    {
        let rows_f32: [f32; 12] = [
