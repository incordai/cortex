...
use cortex_std::{BoolDType, BoolStore, Bytes, Shape, bf16, f16};
use bytemuck::Pod;
...
/// requested output dtype.
///
/// cortex-flex stores bools as 1 byte per element, so only Native and U8 are
...
pub(crate) fn make_bool_tensor(data: Vec<u8>, shape: Shape, out_dtype: BoolDType) -> FlexTensor {
    let store = match out_dtype {
        ...
        BoolDType::U32 => panic!(
            "cortex-flex does not support Bool(U32) storage (only Native and U8). \
...
// / transposed / narrowed) have been migrated to cortex-backend-tests at
