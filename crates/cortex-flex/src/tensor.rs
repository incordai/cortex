...
use cortex_backend::{DType, Element, TensorData, TensorMetadata};
use cortex_std::{Bytes, Shape, bf16, f16};
...
impl FlexTensor {
    ...
    pub fn to_contiguous(&self) -> Self {
        ...
        match self.dtype {
            ...
            DType::Bool(cortex_std::BoolStore::U32) => {
                panic!("cortex-flex: Bool(U32) storage is not yet supported")
            }
...
/// strided iterator. Cortex tensors are capped at 8 dims in practice.
...
fn collapse_for_copy(shape: &[usize], strides: &[isize]) -> CollapsedLayout {
    ...
    // fit our stack buffer. In practice this never triggers (cortex
...
///
/// Matches `cortex_std::DType::size()` semantics: Bool(Native) and Bool(U8) are
...
/// # Panics
///
/// Panics if the dtype has a zero-byte element size. `cortex_std::DType::size()`
/// returns 0 for sub-byte quantized dtypes (Q4F, Q4S, Q2F, Q2S, and most
/// `QuantStore::PackedNative` variants). cortex-flex does not yet support these
...
pub(crate) fn dtype_size(dtype: DType) -> usize {
    // Delegate to cortex-std's canonical size to stay in sync.
    let size = dtype.size();
    assert!(
        size > 0,
        "cortex-flex: dtype {:?} has zero-byte element size (sub-byte packed \
...
mod tests {
    ...
    fn test_to_contiguous_2d_row_stride_gt_col_stride() {
        ...
        let stepped = crate::ops::slice::slice(
            t,
            &[
                cortex_std::Slice::new(0, Some(6), 2),
                cortex_std::Slice::new(0, None, 1),
            ],
        );
