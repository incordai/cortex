...
use cortex::store::RecordError;
use cortex::tensor::{Bytes, Device, Tensor, TensorData};
...
impl LBFGS {
    /// Decompose the optimizer state into a serializable [`OptimizerRecord`] (cortexpack format).
    ///
    ...
    pub fn to_record(&self) -> OptimizerRecord {
        ...
        let tensors = sink
            ...
            .into_iter()
            .map(|(name, data)| {
                cortex_pack::Tensor::new(name, data.dtype, data.shape, None, data.bytes)
            })
            .collect();
...
mod tests {
    ...
    // A cortexpack round-trip of the L-BFGS state (which holds `Vec<Tensor>` history buffers, optional
    // tensors and optional scalars) must restore enough that a further step agrees with the original.
    #[test]
    fn test_lbfgs_cortexpack_round_trip() {
        let device = Device::default().autodiff();
        let tol = 1e-6;
        let x_data = Tensor::<2>::from_data([[1.0], [2.0], [3.0]], &device);
        let y_true = Tensor::<2>::from_data([[3.0], [5.0], [7.0]], &device);
        ...
        assert!(
            ...
            "losses differ after cortexpack round-trip: {loss_original} vs {loss_reloaded}"
        );
    }
