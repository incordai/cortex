...
mod tests {
    ...
    /// A cortexpack round-trip must restore the full state — the moment tensors, amsgrad's optional
    ...
    #[test]
    fn test_adam_state_survives_cortexpack_round_trip() {
        let device = Device::default().autodiff();
        ...
        // Round-trip the optimizer state through the cortexpack format. No device is needed on load:
