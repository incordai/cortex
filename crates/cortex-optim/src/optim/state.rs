//! Decomposition of a state struct into named tensors and scalars for the cortexpack format.
//!
//! Used for both optimizer state — keyed per-[`ParamId`](cortex_core::module::ParamId) rather than
...
//! such a struct into a flat list of named tensors plus a few typed [scalars](cortex_pack::Scalar),
...
//! states and optional nested states. Scalar fields rely on `cortex_pack`'s [`From`]/[`TryFrom`]
//! conversions to [`Scalar`](cortex_pack::Scalar).
...
use cortex_core::tensor::{Device, TensorData};
use cortex_pack::Scalar;

pub use cortex_derive::RecordState;
...
impl StateSource {
    /// Create a source from an existing scalar map (e.g. the cortexpack scalars).
