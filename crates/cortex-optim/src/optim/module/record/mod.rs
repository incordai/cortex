...
use cortex::tensor::Bytes;
use cortex_core as cortex;

use cortex_pack::{Reader, Scalar, Writer};

/// A serialized optimizer state, stored in the [cortexpack](cortex_pack) format.
///
...
/// originating `param_id`) plus a few typed scalar entries kept in the cortexpack scalar map.
///
...
pub struct OptimizerRecord {
    pub(crate) tensors: Vec<cortex_pack::Tensor>,
