...
use cortex_core::module::ParamGroup;
...
/// [`ParamId`](cortex::module::ParamId). Build one with `optimizer.into()` or
    ...
    pub fn to_record(&self) -> OptimizerRecord {
        ...
        for (id, param_state) in self.param_context.iter() {
            ...
            scalars.insert(
                ...
                cortex_pack::Scalar::from(param_state.state.rank()),
            );
            ...
            for (name, data) in sink.tensors {
                tensors.push(cortex_pack::Tensor::new(
                    name,
                    data.dtype,
                    data.shape,
                    Some(id.val()),
...
mod tests {
    ...
    use cortex::tensor::{Distribution, Tensor, Tolerance};
    use cortex_derive::Module;
