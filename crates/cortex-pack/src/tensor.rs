//! Tensor-library-agnostic tensor entry for the cortexpack format.
//!
//! The cortexpack reader and writer operate on [`Tensor`] values, which carry only the
...
//! type) integrates with the rest of the Cortex ecosystem: a reader can hand out
...
use cortex_std::{Bytes, DType, Shape};

/// A single tensor in a cortexpack container, decoupled from any tensor library.
///
