...
//! # Cortex Pack
//!
//! The **cortexpack** binary serialization format for the Cortex deep learning framework.
//!
//! `cortex-pack` is intentionally minimal and tensor-library-agnostic: it depends only on
//! `cortex-std` (for [`DType`] / [`Bytes`]), `serde`, and a CBOR codec. It knows how to read
//! and write the cortexpack container format but has no notion of Cortex modules or tensors.
//! Higher layers (e.g. `cortex-core`) bridge between [`Tensor`] entries and their own
...
//!
//! ```
//! use cortex_pack::{Bytes, DType, Reader, Tensor, Writer};
//!
...
//!     .with_metadata("producer", "cortex-pack docs")
//!     .into_bytes()
//!     .unwrap();
//!
//! // ... and read it back.
//! let reader = Reader::from_bytes(packed).unwrap();
//! assert_eq!(reader.metadata()["producer"], "cortex-pack docs");
...
//! # File format
//!
//! A cortexpack file has three parts: a fixed-size header, a CBOR metadata blob, and a
//! 256-byte-aligned tensor data section. All multi-byte integers are little-endian.
//!
//! ```text
//! ┌──────────────────────────────────────────────────────────────┐
//! │ Header — 10 bytes ([`HEADER_SIZE`])                           │
//! │   magic         : u32  — 0x4255524E "CORTEX" ([`MAGIC_NUMBER`]) │
...
/// The canonical file extension for cortexpack files (without the leading dot).
pub const EXTENSION: &str = "bpk";

// Re-export the core types so callers can build [`Tensor`] entries and inspect descriptors
// without depending on `cortex-std` directly.
pub use cortex_std::{Bytes, DType, Shape};
