...
use cortex_pack::{HEADER_SIZE, Reader, TENSOR_ALIGNMENT, Writer, aligned_data_section_start};
...
fn write_into_matches_to_bytes_and_round_trips() {
    ...
    let reader = Reader::from_bytes(cortex_pack::Bytes::from_bytes_vec(buffer)).unwrap();
