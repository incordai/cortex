//! Write → read round-trip coverage for the cortexpack format.

mod common;

use cortex_pack::{Bytes, DType, Reader, Scalar, Tensor, Writer};
...
fn user_metadata_round_trip() {
    let packed = Writer::new(vec![f32_tensor("w", &[1.0], &[1], None)])
        .with_metadata("producer", "cortex-pack")
        .with_metadata("format", "cortexpack")
        .into_bytes()
        .unwrap();

    let reader = Reader::from_bytes(packed).unwrap();
    assert_eq!(reader.metadata()["producer"], "cortex-pack");
    assert_eq!(reader.metadata()["format"], "cortexpack");
