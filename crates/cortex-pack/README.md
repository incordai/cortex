...
`cortex-pack` reads and writes the cortexpack container format. It is tensor-library-agnostic and
dependency-light: it depends only on [`burn-std`](https://crates.io/crates/burn-std) (for `DType`
/ `Bytes`), `serde`, and a CBOR codec — it knows the on-disk format but has no notion of Cortex
...
If you just want to save and load Cortex models, use the higher-level
...
Reach for `cortex-pack` directly only to produce or consume the raw format.
