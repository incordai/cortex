...
//!
//! Hosts a Cortex server that executes tensor operations on behalf of remote clients. The backend is
...
//! - Composed (protocol): for applications that own their own Iroh router. Cortex hands back only
...
//! cortex_remote::server::RemoteServerBuilder directly; that is also how custom operations
//! (backend extensions) are hosted, over either transport.

use std::sync::Arc;

use crate::Device;
pub use cortex_dispatch::backends::remote::server::{
    AllowAll, AuthorizationRequest, PeerAuthorizer, RemoteProtocol,
};
pub use cortex_dispatch::backends::remote::telemetry;
pub use cortex_dispatch::backends::remote::{Endpoint, RemoteSecret};
pub use cortex_dispatch::devices::CORTEX_REMOTE_ALPN;

use telemetry::TelemetryProbe;

/// Transport used to serve remote clients. Re-exported from `cortex-remote` (via `cortex-dispatch`) so
/// the whole stack shares one definition.
pub use cortex_dispatch::remote_server::Channel;

/// Build Cortex's protocol handler for `device`'s backend.
///
/// Returns a builder to optionally attach telemetry and an authorizer before calling `build`.
/// Register the result on an application-owned Iroh router under CORTEX_REMOTE_ALPN.
...
impl<'a> RemoteProtocolBuilder<'a> {
    ...
    pub fn build(self) -> RemoteProtocol {
        cortex_dispatch::remote_server::remote_protocol(
...
pub fn start(device: Device, channel: Channel) {
    cortex_dispatch::remote_server::start(device.into_dispatch(), channel)
}
...
pub async fn start_async(device: Device, channel: Channel) {
    cortex_dispatch::remote_server::start_async(device.into_dispatch(), channel).await
}
