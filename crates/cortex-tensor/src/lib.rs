...
//! generic methods on `Tensor<D, K>` that need to call into `cortex_dispatch` are
...
//! `DispatchTensor` or other `cortex_dispatch` types appear in their
...
//! any `cortex_dispatch` types. Downstream crates that monomorphize the public
...
pub(crate) use cortex_backend::TensorPrimitive;
