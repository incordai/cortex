use alloc::vec::Vec;
use cortex_backend::{TensorMetadata, TensorPrimitive, get_device_settings};
use cortex_dispatch::{Dispatch, DispatchTensor};
...
/// cortex-tensor's own ops keep working without leaking the dispatch types).
macro_rules! ext_fn {
    ($(#[$meta:meta])* fn $($tt:tt)+) => {
...
impl BridgeTensor {
    ...
    pub fn dtype(&self) -> cortex_std::DType {
        match self.as_variant() {
    ...
    pub fn shape(&self) -> cortex_std::Shape {
        match self.as_variant() {
