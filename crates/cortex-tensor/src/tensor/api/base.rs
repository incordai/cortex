...
use cortex_std::ExecutionError;
use cortex_std::{SliceOps, sync::RwLock};
    ...
    K: Basic,
{
    ...
    /// let tensor = Tensor::<1, cortex_tensor::Int>::arange(0..10, &device);
    ...
    /// # Example
    /// ```rust
    /// use cortex_tensor::{Tensor, TensorData, s};
    ///
    /// let device = Default::default();
    /// let tensor = Tensor::<2>::from_data(
    ///     [
...
impl DataIterFmt {
    fn next(&self) -> String {
        match self.data.dtype {
            ...
            DType::F16 => fmt_float(self.next_elem::<cortex_std::f16>(), self.precision),
            DType::BF16 => fmt_float(self.next_elem::<cortex_std::bf16>(), self.precision),
            ...
            DType::Bool(store) => match store {
                cortex_std::BoolStore::Native => fmt_elem(self.next_elem::<bool>()),
                cortex_std::BoolStore::U8 => fmt_elem(self.next_elem::<u8>().to_bool()),
                cortex_std::BoolStore::U32 => fmt_elem(self.next_elem::<u32>().to_bool()),
            },
...
// so it is compiled exactly once inside `cortex-tensor` instead of being
...
/// state-machine code lives here, compiled once inside `cortex-tensor`.
...
fn display_fmt_inner(
    ...
    for i in start..end {
        ...
        let data = cortex_std::reader::try_read_sync(into_data_async_impl(sliced, kind));
