...
use cortex_backend::{DeviceOps, distributed::DistributedOps};
use cortex_dispatch::{Dispatch, DispatchTensor};
pub use cortex_std::distributed::*;
...
pub fn all_reduce<const D: usize>(
    ...
    // Safety: we call `assume_resolved` only to wrap it in `cortex_tensor`'s [CollectiveTensor].
