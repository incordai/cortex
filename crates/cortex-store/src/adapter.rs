...
//! - PyTorch/Cortex format conversion (weight transposition, parameter renaming)
...
use cortex_core::tensor::shape;
use cortex_core::tensor::{DType, TensorData};
...
/// ```rust
/// # use cortex_store::HalfPrecisionAdapter;
...
/// ```rust
/// # use cortex_store::{FloatCastAdapter, ModuleAdapter, PyTorchToCortexAdapter};
/// # use cortex_core::tensor::DType;
/// let adapter = PyTorchToCortexAdapter.chain(FloatCastAdapter::to(DType::F16));
/// ```
#[derive(Debug, Clone)]
...
impl ModuleAdapter for CortexToPyTorchAdapter {
    ...
    fn get_alternative_param_name(&self, param_name: &str, container_type: &str) -> Option<String> {
        ...
        if is_normalization_layer(container_type) {
            pytorch_norm_param_to_cortex(param_name).map(|s| s.to_string())
        } else {
            None
        }
    }
...
fn pytorch_norm_param_to_cortex(param_name: &str) -> Option<&'static str> {
    match param_name {
        "weight" => Some("gamma"),
        "bias" => Some("beta"),
        _ => None,
...
fn adapt_pytorch_tensor(
    ...
    if is_normalization_layer(&module_type) {
        let new_name = match direction {
            PyTorchConversionDirection::PyTorchToCortex => pytorch_norm_param_to_cortex(param_name),
...
mod tests {
    ...
    use alloc::rc::Rc;
    use alloc::sync::Arc;
    use cortex_core::tensor::{DType, Shape, TensorData};
    use core::sync::atomic::{AtomicUsize, Ordering};

    #[test]
    fn test_module_names_match_cortex_nn() {
        // If these types are renamed or moved in `cortex-nn`, this test will fail to compile.
    ...
    fn test_half_precision_chain() {
        let adapter = PyTorchToCortexAdapter.chain(HalfPrecisionAdapter::new());
    ...
    fn test_half_precision_skips_non_float() {
        use cortex_core::tensor::quantization::QuantScheme;
