...
use alloc::vec::Vec;
use cortex_core::tensor::Bytes;
use cortex_pack::{Error as PackError, Reader, Tensor as PackTensor, Writer};

/// Store mode for CortexpackStore
enum StoreMode {
    #[cfg(feature = "std")]
    File(PathBuf),
    Bytes(Option<Bytes>),
...
/// CortexpackStore - A Cortex-specific file format store using CBOR for metadata
pub struct CortexpackStore {
    ...
    /// Adapter applied when loading (source -> Cortex)
    from_adapter: Box<dyn ModuleAdapter>,
    /// Adapter applied when saving (Cortex -> target)
...
impl CortexpackStore {
    ...
    pub fn default_metadata() -> BTreeMap<String, String> {
        ...
        metadata.insert("format".into(), "cortexpack".into());
    ...
    ///
    /// ```no_run
    /// # use cortex_store::CortexpackStore;
    /// // Automatically appends .bpk
    /// let store = CortexpackStore::from_file("model");  // creates "model.bpk"
    ///
    /// // Already has extension, no append
    /// let store = CortexpackStore::from_file("model.bpk");  // uses "model.bpk"
    /// let store = CortexpackStore::from_file("model.myext");  // uses "model.myext"
    ///
    /// // Disable auto-extension
    /// let store = CortexpackStore::from_file("model").auto_extension(false);  // uses "model"
    /// ```
    #[cfg(feature = "std")]
    ...
    /// let store = CortexpackStore::from_static(MODEL_DATA);
    /// ```
    pub fn from_static(data: &'static [u8]) -> Self {
        use cortex_core::tensor::AllocationProperty;
    ...
    /// let store = CortexpackStore::from_file("model");  // -> "model.bpk"
    ///
    /// // With auto_extension disabled
    /// let store = CortexpackStore::from_file("model")
...
impl ModuleStore for CortexpackStore {
    ...
    fn collect_from<M: ModuleSnapshot>(&mut self, module: &M) -> Result<(), Self::Error> {
        ...
        // Bridge snapshots to tensor-agnostic cortexpack entries (materializing their data)
    ...
    fn apply_to<M: ModuleSnapshot>(
        ...
        // Cortexpack is Cortex's native format, so no enum variant skipping needed
