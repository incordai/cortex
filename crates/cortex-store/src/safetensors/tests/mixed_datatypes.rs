...
use cortex_core::tensor::{Bool, Device, Int, Tensor};
use cortex_nn as nn;
...
mod tests {
    use cortex_core::tensor::{BoolStore, DType};
    ...
    fn test_dtype_detection() {
        ...
        for snapshot in snapshots {
            ...
            if path.contains("float_tensor") || path.contains("linear") {
                ...
                assert_eq!(
                    dtype,
                    cortex::tensor::DType::Bool(BoolStore::Native),
    ...
    fn test_mixed_precision_integers() {
        ...
        struct MultiIntModel {
            // Note: Cortex's Tensor<D, Int> uses the backend's default int type
