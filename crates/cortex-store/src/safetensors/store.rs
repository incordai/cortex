...
use cortex_core::tensor::{BoolStore, DType, TensorData};
use core::fmt;
use core::ops::Deref;
...
fn safetensors_to_snapshots_lazy(
    ...
    for (name, tensor_snapshot) in tensors.tensors() {
        ...
        let dtype = safetensor_dtype_to_cortex(tensor_snapshot.dtype())?;
        ...
        let data_fn = alloc::rc::Rc::new(move || {
            ...
            let bytes = cortex_core::tensor::Bytes::from_bytes_vec(tensor.data().to_vec());
            Ok(TensorData {
                bytes,
                shape: tensor.shape().into(),
                dtype: safetensor_dtype_to_cortex(tensor.dtype())
...
fn safetensors_to_snapshots_lazy_file(
    ...
    for (name, tensor_snapshot) in tensors.tensors() {
        ...
        let data_fn = alloc::rc::Rc::new(move || {
            ...
            Ok(TensorData {
                bytes: cortex_core::tensor::Bytes::from_bytes_vec(tensor.data().to_vec()),
                shape: tensor.shape().into(),
                dtype: safetensor_dtype_to_cortex(tensor.dtype())
                    .map_err(|_| crate::TensorSnapshotError::DataError("Invalid dtype".into()))?,
            })
...
fn safetensor_dtype_to_cortex(dtype: safetensors::Dtype) -> Result<DType, SafetensorsStoreError> {
