...
//! Benchmark comparing different loading modes for CortexpackStore.
//!
...
use cortex_core::tensor::{AllocationProperty, Bytes};
use cortex_nn as nn;
use cortex_store::{
    CortexpackStore, ModuleSnapshot, ModuleStore, PyTorchToCortexAdapter, SafetensorsStore,
};
...
/// Get path to Cortexpack model file
fn get_cortexpack_path() -> PathBuf {
    get_model_dir().join("large_model.bpk")
}

/// Generate Cortexpack file from existing SafeTensors file if needed
fn ensure_cortexpack_file() {
    let bp_path = get_cortexpack_path();
    ...
    println!("⏳ Generating Cortexpack file from SafeTensors...");
    ...
    // Save as Cortexpack
    let mut cortexpack_store = CortexpackStore::from_file(&bp_path);
    model
        .save_into(&mut cortexpack_store)
        .expect("Failed to save as Cortexpack");

    println!("✅ Created Cortexpack file: {}", bp_path.display());
}
...
fn get_static_model_bytes() -> &'static [u8] {
    STATIC_MODEL_BYTES.get_or_init(|| {
        let bp_path = get_cortexpack_path();
        let bytes = fs::read(&bp_path).expect("Failed to read Cortexpack file");
...
fn main() {
    // Ensure Cortexpack file exists
    ensure_cortexpack_file();

    let bp_path = get_cortexpack_path();
    let file_size = fs::metadata(&bp_path).unwrap().len() as f64 / 1_048_576.0;

    println!("✅ Found Cortexpack model file:");
            ...
            fn file(bencher: Bencher) {
                ...
                bencher
                    ...
                    .bench(|| {
                        let device = $device;
                        let mut model = LargeModel::new(&device);
                        let mut store = CortexpackStore::from_file(&bp_path);
            ...
            fn static_bytes(bencher: Bencher) {
                ...
                bencher
                    ...
                    .bench(|| {
                        ...
                        let mut store = CortexpackStore::from_bytes(Some(bytes));
                        model.load_from(&mut store).expect("Failed to load");
                    });
            }
            ...
            fn static_zero_copy(bencher: Bencher) {
                ...
                bencher
                    ...
                    .bench(|| {
                        ...
                        let mut store = CortexpackStore::from_static(static_bytes);
                        model.load_from(&mut store).expect("Failed to load");
                    });
            }
...
mod verification {
    ...
    fn verify_static_vs_bytes_equality() {
        ...
        let mut store_zc = CortexpackStore::from_static(static_bytes);
        model_zc
        ...
        let bytes = Bytes::from_bytes_vec(static_bytes.to_vec());
        let mut store_copy = CortexpackStore::from_bytes(Some(bytes));
        model_copy
