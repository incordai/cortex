...
//!
//! Usage:
//!   cargo run -p cortex-store --example half_precision

use cortex_core::module::Module;
use cortex_core::{self as cortex, tensor::Device};
use cortex_nn::{LayerNorm, LayerNormConfig, Linear, LinearConfig};
use cortex_store::{CortexpackStore, HalfPrecisionAdapter, ModuleSnapshot};
...
fn main() {
    ...
    let mut store = CortexpackStore::from_file(path_f32.to_str().unwrap()).overwrite(true);
    ...
    let mut store = CortexpackStore::from_file(path_f16.to_str().unwrap())
        .overwrite(true)
    ...
    let mut store = CortexpackStore::from_file(path_mixed.to_str().unwrap())
        .overwrite(true)
    ...
    let mut load_store =
        CortexpackStore::from_file(path_f16.to_str().unwrap()).with_from_adapter(adapter);
