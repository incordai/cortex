//! Example: Generate a Cortexpack file for inspection
//!
//! This example creates a simple Cortexpack file that you can examine to understand the format.
//!
//! Usage:
//!   cargo run --example cortexpack-inspect [output_path]
//!
//! Example:
//!   cargo run --example cortexpack-inspect sample.bpk
//!   cargo run --example cortexpack-inspect /tmp/test.bpk
//!
//! After generating the file, examine it with:
//!   hexdump -C sample.bpk | head -100
//!   xxd sample.bpk | head -100
//!   hexyl sample.bpk
use cortex_core::{self as cortex, tensor::Device};

use cortex_core::module::Module;
use cortex_nn::{Linear, LinearConfig};
use cortex_store::{CortexpackStore, ModuleSnapshot};
use std::env;
...
fn main() {
    ...
    println!("Creating sample Cortexpack file: {}", output_path);
    println!();
    ...
    // Save to Cortexpack format with metadata
    let mut store = CortexpackStore::from_file(&output_path)
        .overwrite(true)
        .metadata("format", "cortexpack")
        .metadata("description", "Sample file for examining Cortexpack format")
    ...
    println!("  ├─────────────────────────────────────┤");
    println!("  │ - Magic: 0x4E525542 (CORTEX in LE)   │");
