...
- **Cortexpack Format** - Native Cortex format with CBOR metadata, memory-mapped loading, ParamId
...
- **Half-Precision Storage** - Automatic F32/F16 conversion with smart defaults for reduced model
  file size
- **No-std Support** - Cortexpack and SafeTensors formats available in embedded and WASM environments

## Quick Start

```rust
use cortex_store::{ModuleSnapshot, PytorchStore, SafetensorsStore, CortexpackStore, HalfPrecisionAdapter};
...
See the **[Cortex Book - Saving and Loading](../../cortex-book/src/saving-and-loading.md)** chapter.
