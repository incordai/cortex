...
```rust
use cortex::record::{FullPrecisionSettings, Recorder};
use cortex_import::safetensors::{AdapterType, LoadArgs, SafetensorsFileRecorder};
...
```rust
use cortex_import::safetensors::{AdapterType, LoadArgs};
...
// Save to Cortexpack (native format)
let mut store = CortexpackStore::from_file("output.bpk");
model.save_into(&mut store)?;
```
