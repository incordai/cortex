...
/// # `backend_extension`
///
/// Attribute macro that generates dispatch glue for Cortex backend extension traits.
///
/// ## Usage
///
/// ```rust,ignore
/// use cortex_backend_extension::backend_extension;
///
...
impl TensorKind {
    fn from_type(ty: &Type) -> Option<Self> {
        match ty {
            ...
            // Handle simple paths: Float, FloatTensor<Self>, cortex::...::FloatTensor<Self>
            Type::Path(tp) => {
                let last = tp.path.segments.last()?.ident.to_string();
    ...
    fn to_primitive_ty(self) -> TokenStream2 {
        match self {
            Self::Float => quote! { cortex::backend::tensor::FloatTensor<Self> },
            Self::Int => quote! { cortex::backend::tensor::IntTensor<Self> },
            Self::Bool => quote! { cortex::backend::tensor::BoolTensor<Self> },
            Self::Quantized => quote! { cortex::backend::tensor::QuantizedTensor<Self> },
        }
    }
...
fn expand_extension(ir: Extension, mut original_trait: ItemTrait) -> TokenStream2 {
    ...
    quote! {
        ...
        impl #trait_name for cortex::backend::Dispatch {
            #( #dispatch_methods )*
        }
    }
}
...
fn gen_tensor_input_dispatch_body(ir: &Extension, op: &Operation) -> TokenStream2 {
        ...
        op.inputs
            ...
            .filter_map(|a| match &a.kind {
                ...
                ArgKind::Extension(ty) => {
                    let n = &a.name;
                    let target_ty = struct_ty_with_param(ty, quote! { cortex::backend::Dispatch });
                    ...
                    Some(quote! {
                        <#target_ty as cortex::backend::ExtensionType<cortex::backend::Dispatch>>::#method(&#n)
                    })
                }
    ...
    let chain = |opts: Vec<TokenStream2>| -> TokenStream2 {
        match opts.split_first() {
            None => quote! { Option::<&cortex::backend::DispatchTensor>::None },
    ...
    let concrete_tag_arms = ir.backends.concrete.iter().enumerate().map(|(i, backend)| {
        ...
        quote! {
            #cfg_attr
            cortex::backend::DispatchTensorKind::#b_ident(_) => (false, #i),
        }
    });
    let ad_tag_arm = has_ad.then(|| {
        let inner = ir.backends.concrete.iter().enumerate().map(|(i, backend)| {
            ...
            quote! {
                #cfg_attr
                cortex::backend::DispatchTensorKind::#b_ident(_) => #i,
            }
        });
        quote! {
            #ad_cfg_attr
            cortex::backend::DispatchTensorKind::Autodiff(inner) => (true, match inner.as_ref() {
                #( #inner )*
                #[allow(unreachable_patterns)]
                _ => usize::MAX,
            }),
    ...
    let concrete_call_arms = ir.backends.concrete.iter().enumerate().map(|(i, backend)| {
        ...
        let pre_extract = op.inputs.iter().filter_map(|a| match &a.kind {
            ArgKind::Tensor(_) => {
                ...
                Some(quote! {
                    let #n = match #n.kind {
                        cortex::backend::DispatchTensorKind::#b_ident(bt) => bt,
                        #[allow(unreachable_patterns)]
                        _ => #mismatch,
                    };
                })
    ...
    quote! {
        ...
        let (checkpointing, __cortex_backend_tag): (
            Option<cortex::backend::CheckpointingStrategy>,
            (bool, usize),
        ) = {
            let __repr: &cortex::backend::DispatchTensor = (#float_chain)
                .or_else(|| #any_chain)
...
fn gen_tensor_input_ad_arm(
    ...
    let unwraps = op.inputs.iter().filter_map(|a| match &a.kind {
        ...
        ArgKind::Tensor(TensorKind::Float) => {
            ...
            Some(quote! {
                let #n = match #n.kind {
                    cortex::backend::DispatchTensorKind::Autodiff(inner) => match *inner {
                        cortex::backend::DispatchTensorKind::#b_ident(bt) => bt.autodiff(),
                        #[allow(unreachable_patterns)]
                        _ => #mismatch,
                    },
                    #[allow(unreachable_patterns)]
        ...
        ArgKind::Tensor(kind) => {
            ...
            Some(quote! {
                let #n = match #n.kind {
                    cortex::backend::DispatchTensorKind::#b_ident(bt) => bt.#method(),
                    #[allow(unreachable_patterns)]
        ...
        ArgKind::Extension(ty) => {
            ...
            Some(quote! {
                let #n = <#ad_ty as cortex::backend::ExtensionType<Autodiff<#b_ident>>>::map_from_dispatch(
                    #n,
                    |kind| {
                        let bt = match kind {
                            cortex::backend::DispatchTensorKind::Autodiff(inner) => match *inner {
...
fn gen_output_wrap(op: &Operation, b_ident: &Ident, is_ad: bool) -> TokenStream2 {
    ...
    let wrap_custom = |accessor: TokenStream2| -> TokenStream2 {
        if is_ad {
            quote! {
                cortex::backend::ExtensionType::map_to_dispatch(
                    #accessor,
                    |tensor| match tensor {
                        cortex::backend::BackendTensor::Float(t) => cortex::backend::DispatchTensorKind::Autodiff(
                            Box::new(cortex::backend::DispatchTensorKind::#b_ident(
                                cortex::backend::BackendTensor::Autodiff(t),
                            )),
                        ),
                        cortex::backend::BackendTensor::Int(t) => cortex::backend::DispatchTensorKind::#b_ident(cortex::backend::BackendTensor::Int(t)),
                        cortex::backend::BackendTensor::Bool(t) => cortex::backend::DispatchTensorKind::#b_ident(cortex::backend::BackendTensor::Bool(t)),
                        cortex::backend::BackendTensor::Quantized(t) => cortex::backend::DispatchTensorKind::#b_ident(cortex::backend::BackendTensor::Quantized(t)),
                        #[allow(unreachable_patterns)]
            ...
            quote! {
                cortex::backend::ExtensionType::map_to_dispatch(
                    #accessor,
                    |tensor| cortex::backend::DispatchTensorKind::#b_ident(tensor),
                    checkpointing,
                )
            }
        }
    ...
    match &op.output {
        OperationOutput::Tensor(kind) => {
            ...
            quote! { cortex::backend::DispatchTensor { kind: #wrapped, checkpointing } }
        }
        OperationOutput::Tuple(elems) => {
...
fn gen_backend_call(ir: &Extension, op: &Operation, backend: &Backend) -> TokenStream2 {
    ...
    let unwraps = op.inputs.iter().filter_map(|a| match &a.kind {
        ...
        ArgKind::Extension(ty) => {
            ...
            Some(quote! {
                let #name = <#b_ty as cortex::backend::ExtensionType<#b_ident>>::map_from_dispatch(
                    #name,
                    |kind| match kind {
                        cortex::backend::DispatchTensorKind::#b_ident(bt) => bt,
                        #[allow(unreachable_patterns)]
...
fn gen_tensor_wrap(
    ...
    if is_ad && *kind == TensorKind::Float {
        quote! {
            cortex::backend::DispatchTensorKind::Autodiff(
                Box::new(cortex::backend::DispatchTensorKind::#b_ident(
                    cortex::backend::BackendTensor::Autodiff(#val)
                ))
            )
        }
    } else {
        quote! {
            cortex::backend::DispatchTensorKind::#b_ident(
                cortex::backend::BackendTensor::#variant(#val)
            )
        }
    }
}
...
fn gen_repr_arm(case: &DeriveCase, float_only: bool) -> TokenStream2 {
    ...
    let (needed, expr): (Vec<usize>, TokenStream2) = if let Some(i) = float_i {
        ...
        let calls = ext_is.iter().map(|&i| {
            let bind = &case.fields[i].bind;
            let dispatch_ty = struct_ty_with_param(&case.fields[i].ty, quote! { cortex::backend::Dispatch });
            quote! { .or_else(|| <#dispatch_ty as cortex::backend::ExtensionType<cortex::backend::Dispatch>>::#method(#bind)) }
        });
        (
            ext_is.clone(),
            quote! { Option::<&cortex::backend::DispatchTensor>::None #( #calls )* },
        )
    } else {
        (
            Vec::new(),
            quote! { Option::<&cortex::backend::DispatchTensor>::None },
        )
    };
...
pub fn derive_extension_type(input: TokenStream) -> TokenStream {
        ...
        let exprs: Vec<_> = case
            ...
            .map(|f| {
                ...
                if f.is_ext {
                    ...
                    let variant = kind.variant();
                    quote! {
                        cortex::backend::DispatchTensor {
                            kind: map_kind(cortex::backend::BackendTensor::#variant(#bind)),
                            checkpointing,
                        }
                    }
                } else {
    ...
    let unwrap_arms = cases.iter().map(|case| {
        ...
        let exprs: Vec<_> = case
            ...
            .map(|f| {
                let bind = &f.bind;
                if f.is_ext {
                    quote! { cortex::backend::ExtensionType::map_from_dispatch(#bind, &unwrap_kind) }
                } else if let Some(kind) = f.tensor_kind {
                    let method = kind.unwrap_method();
    ...
    TokenStream::from(quote! {
        impl #impl_generics cortex::backend::ExtensionType<B> for #name #ty_generics #where_clause {
            type Target = #name<cortex::backend::Dispatch>;

            #[allow(unused_variables)]
            fn map_to_dispatch<F>(
                self,
                map_kind: F,
                checkpointing: Option<cortex::backend::CheckpointingStrategy>,
            ) -> Self::Target
            where
                F: Fn(cortex::backend::BackendTensor<B>) -> cortex::backend::DispatchTensorKind,
            {
            ...
            where
                F: Fn(cortex::backend::DispatchTensorKind) -> cortex::backend::BackendTensor<B>,
            {
                match target { #( #unwrap_arms )* }
            }

            fn dispatch_repr(target: &Self::Target) -> Option<&cortex::backend::DispatchTensor> {
                match target { #( #any_repr_arms )* }
            }

            fn dispatch_float_repr(target: &Self::Target) -> Option<&cortex::backend::DispatchTensor> {
