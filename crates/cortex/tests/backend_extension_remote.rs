...
use cortex::backend::Autodiff;

#[derive(ExtensionType)]
pub struct Pair<B: cortex::backend::Backend> {
    pub a: FloatTensor<B>,
    pub b: IntTensor<B>,
}
...
#[derive(ExtensionType)]
pub struct TupleInputs<B: cortex::backend::Backend>(pub FloatTensor<B>, pub IntTensor<B>);
...
#[derive(ExtensionType)]
pub enum Operand<B: cortex::backend::Backend> {
    Dense(FloatTensor<B>),
    Sparse {
...
// Nested `#[extension_type]` field plus a non-tensor passthrough field, mirroring cortex-vision's
...
#[derive(ExtensionType)]
pub struct Coords<B: cortex::backend::Backend> {
    pub left: IntTensor<B>,
    pub top: IntTensor<B>,
}

#[derive(ExtensionType)]
pub struct Stats<B: cortex::backend::Backend> {
...
pub trait AdGatedBackend: cortex::backend::Backend {
...
impl AdGatedBackend for cortex::backend::Autodiff<Remote> {
    fn ad_gated(_pair: Pair<Self>) -> FloatTensor<Self> {
...
#[backend_extension(Remote)]
pub trait NestedBackend: cortex::backend::Backend {
...
pub trait EnumBackend: cortex::backend::Backend {
...
#[cfg(feature = "autodiff")]
impl EnumBackend for cortex::backend::Autodiff<Remote> {
    fn use_operand(_op: Operand<Self>) -> FloatTensor<Self> {
...
pub trait GatedBackend: cortex::backend::Backend {
...
mod autodiff_struct_input {
    ...
    #[backend_extension(Autodiff, Remote)]
    pub trait AdBackend: cortex::backend::Backend {
