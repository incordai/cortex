...
use cortex::backend::{Dispatch, ExtensionType, NdArray, backend_extension, tensor::FloatTensor};
...
pub trait RtBackend: cortex::backend::Backend {
...
mod multi_backend_autodiff {
    use super::*;
    use cortex::backend::autodiff::checkpoint::strategy::CheckpointStrategy;
    use cortex::backend::{Autodiff, Backend, Flex};
    ...
    fn assert_double(device: Device) {
        ...
        Tensor::<1>::from_dispatch(out)
            .into_data()
            .assert_eq(&cortex::tensor::TensorData::from([2.0f32, 4.0, 6.0]), true);
    }
...
mod autodiff_gradients {
    ...
    use cortex::backend::ops::FloatTensorOps;
    use cortex::tensor::TensorData;

    #[derive(ExtensionType)]
    pub struct FloatPair<B: cortex::backend::Backend> {
        pub x: FloatTensor<B>,
        pub y: FloatTensor<B>,
    }

    #[backend_extension(Autodiff, NdArray)]
    pub trait GradBackend: cortex::backend::Backend {
