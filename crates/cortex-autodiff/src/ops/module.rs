...
use cortex_backend::{Backend, get_device_settings};
use cortex_std::{IntDType, Slice};
...
impl<B: Backend, C: CheckpointStrategy> ModuleOps<Autodiff<B, C>> for Autodiff<B, C> {
    ...
    fn ctc_loss(
        ...
        if !B::has_ctc_loss_backward() {
            return cortex_backend::ops::ctc::ctc_loss_default::<Self>(
                log_probs,
                targets,
                input_lengths,
                target_lengths,
...
// adapted from: cortex_cubecl::kernel::fft::base::pad_to_length
