...
pub fn ctc_grad_from_alpha_beta_default<B: Backend>(
    ...
    let int_dtype: cortex_std::IntDType = targets.dtype().into();
...
impl<B: Backend> AlphaCtx<B> {
    fn compute(
        ...
        let max_target_len = target_shape.dims::<2>()[1];
        let device = log_probs.device();
        let float_dtype: cortex_std::FloatDType = log_probs.dtype().into();
        let int_dtype: cortex_std::IntDType = targets.dtype().into();
        let settings = get_device_settings::<B>(&device);
...
fn log_sum_exp<B: Backend>(
    a: FloatTensor<B>,
    b: FloatTensor<B>,
    bool_dtype: cortex_std::BoolDType,
) -> FloatTensor<B> {
