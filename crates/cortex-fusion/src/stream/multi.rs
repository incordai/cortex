...
use cortex_ir::{HandleContainer, OperationIr, TensorId};
...
impl<R: FusionRuntime> MultiStream<R> {
    ...
    pub fn mark_read(
        &mut self,
        id: StreamId,
        ir: &cortex_ir::TensorIr,
        handles: &HandleContainer<R::FusionHandle>,
    ) {
        if !matches!(ir.status, cortex_ir::TensorStatus::ReadWrite) {
            return;
        };
    ...
    pub(crate) fn defer_free(&mut self, id: StreamId, ir: cortex_ir::TensorIr) {
        if let Some(stream) = self.streams.get_mut(&id) {
...
mod tests {
    ...
    use cortex_backend::{DType, DeviceId, DeviceOps, DeviceSettings, Shape};
    use cortex_ir::{FloatOperationIr, TensorIr, TensorStatus, UnaryOpIr};
    use cortex_std::{BoolDType, FloatDType, IntDType, device::Device};
