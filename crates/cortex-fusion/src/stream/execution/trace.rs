...
use cortex_ir::{OperationIr, TensorIr};
use cortex_std::config::{fusion::FusionLogLevel, log_fusion};
use core::fmt::Write;
...
fn write_dtype(out: &mut String, dtype: &cortex_backend::DType) {
...
fn write_shape(out: &mut String, shape: &cortex_backend::Shape) {
    out.push('[');
    let mut first = true;
    for d in shape.iter() {
        if !first {
...
mod tests {
    ...
    use cortex_ir::{BaseOperationIr, TensorId, TensorIr, TensorStatus, UnaryOpIr};
    ...
    fn op_kind_includes_inner_variant() {
        ...
        let op = OperationIr::BaseFloat(BaseOperationIr::Permute(cortex_ir::PermuteOpIr {
            input,
            out,
            axes: vec![1, 0],
        }));
    ...
    fn op_kind_handles_unary_inside_float() {
        ...
        let op = OperationIr::Float(
            DType::F32,
            cortex_ir::FloatOperationIr::Exp(UnaryOpIr { input, out }),
        );
    ...
    fn format_table_puts_name_and_score_in_block_header() {
        let op = OperationIr::Float(
            DType::F32,
            cortex_ir::FloatOperationIr::Exp(UnaryOpIr {
                input: tensor(1, &[4]),
                out: tensor(2, &[4]),
            }),
        );
