...
pub use cortex_cubecl_fusion::{CubeFusionHandle, FallbackOperation};
use cortex_fusion::UnfusedOp;
use cortex_fusion::{
...
mod registry;
pub use cortex_cubecl_fusion::optim::{CubeOptimization, CubeOptimizationState, FusedOperation};
pub use registry::{
    BUILTIN_NAMES, CubeFuser, OptimizationProvider, RegistryError, register, remove,
};

impl<R> cortex_fusion::Optimization<FusionCubeRuntime<R>> for CubeOptimization<R>
where
    R: CubeRuntime,
{
    fn execute(
...
{
    fn run(&self, context: &mut cortex_fusion::stream::Context<CubeFusionHandle<R>>) {
