...
pub(crate) fn from_data<R: CubeRuntime>(data: TensorData, device: &R::Device) -> CubeTensor<R> {
    ...
    if data.bytes.property() == cortex_std::AllocationProperty::Device {
        let _ = data.bytes.read(cortex_std::Reader::new());
    }
...
pub(crate) async fn into_data<R: CubeRuntime>(
    ...
    let read = match cortex_std::runtime_kind() {
        cortex_std::RuntimeKind::Async => tensor.client.read_one_tensor_async(binding).await,
