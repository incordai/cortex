...
//!
//! Wrapping a router backend as [`Fusion`](cortex_fusion::Fusion) turns recurring groups of tensor
...
//!
//! `cortex-fusion` names its hook `Optimization`; here that hook *is* a cached graph execution
...
use cortex_fusion::stream::{Context, Operation, OrderedExecution};
use cortex_fusion::{
...
// `impl BackendIr for Fusion<B>` in cortex-fusion).
impl<R: RouterChannel> BackendIr for BackendRouter<R> {
    type Handle = RouterTensor<R::Client>;

    fn float_tensor(handle: TensorHandle<Self::Handle>) -> cortex_backend::tensor::FloatTensor<Self> {
        handle.handle
    }

    fn int_tensor(handle: TensorHandle<Self::Handle>) -> cortex_backend::tensor::IntTensor<Self> {
        handle.handle
    }

    fn bool_tensor(handle: TensorHandle<Self::Handle>) -> cortex_backend::tensor::BoolTensor<Self> {
        handle.handle
    }

    fn quantized_tensor(
        handle: TensorHandle<Self::Handle>,
    ) -> cortex_backend::tensor::QuantizedTensor<Self> {
        handle.handle
    }

    fn float_tensor_handle(tensor: cortex_backend::tensor::FloatTensor<Self>) -> Self::Handle {
        tensor
    }

    fn int_tensor_handle(tensor: cortex_backend::tensor::IntTensor<Self>) -> Self::Handle {
        tensor
    }

    fn bool_tensor_handle(tensor: cortex_backend::tensor::BoolTensor<Self>) -> Self::Handle {
        tensor
    }

    fn quantized_tensor_handle(
        tensor: cortex_backend::tensor::QuantizedTensor<Self>,
    ) -> Self::Handle {
        tensor
    }
}
...
impl<R: RouterChannel> FusionBackend for BackendRouter<R> {
    ...
    fn cast_float(tensor: cortex_backend::tensor::FloatTensor<Self>, dtype: DType) -> Self::Handle {
...
/// it unfused (a one-op segment — e.g. a source op whose output is read right away) `cortex-fusion`
