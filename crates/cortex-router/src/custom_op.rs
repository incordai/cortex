...
use alloc::sync::Arc;
use cortex_backend::backend::BackendTypes;
use cortex_ir::{BackendIr, CustomOpIr, HandleContainer};
...
mod tests {
    ...
    use cortex_backend::{DType, Scalar, Shape, TensorData, ops::FloatTensorOps};
    use cortex_flex::Flex;
    use cortex_ir::{OperationIr, ScalarIr, TensorId, TensorIr};
    use std::sync::Mutex;
    ...
    fn custom_op_can_create_tensors_from_the_device() {
        ...
        let data = interp.get_tensor(&TensorIr {
            status: cortex_ir::TensorStatus::ReadOnly,
            ..out
        });
        match data {
            cortex_ir::HandleKind::Int(_) => {}
